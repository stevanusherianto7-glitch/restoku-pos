...
describe('buildMenuUrl', () => {
    it('builds url from base + slug', () => {
        expect(buildMenuUrl('https://restoku.id', 'senopati')).toBe('https://restoku.id/m/senopati');
    });

    it('strips trailing slash on base', () => {
        expect(buildMenuUrl('https://restoku.id/', 'kedai-nusantara')).toBe('https://restoku.id/m/kedai-nusantara');
    });
