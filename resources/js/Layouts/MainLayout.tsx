...
const nav: NavGroup[] = [
  {
    ...
    items: [
      ...
      { name:"Monitor Pesanan", href:"/monitor-pesanan", phase:1, roles:["kasir","waiter","manager","owner"] },
  ...
  },
  {
    ...
    items: [
      ...
      { name:"Manajemen Meja",    href:"/manajemen-meja",    phase:1, roles:["kasir","waiter","manager","owner"] },
    ],
  },
  {
    title: "Inventaris", Icon: Boxes,
    roles: ["manager","owner"],
    items: [
  ...
  {
    title: "Operasional", Icon: BriefcaseBusiness,
...
function Sidebar({ isCollapsed, setIsCollapsed }: { isCollapsed: boolean; setIsCollapsed: (v: boolean) => void; }) {
  ...
  useEffect(() => {
    ...
    if (raw) {
      try { setActiveKaryawan(JSON.parse(raw)); } catch { /* ignore */ }
    }
  }, []);
    ...
    const fetchReservations = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const pending = data.filter((r: any) => r.status === "Menunggu Konfirmasi" || r.status === "pending");
  ...
  return (
    <aside className={`flex h-full shrink-0 flex-col p-4 transition-all duration-500 relative ${asideBg} ${isCollapsed ? "w-[80px]" : "w-64"}`}>
      {/* Brand & Toggle */}
      <div className={`mb-6 flex items-center ${isCollapsed ? "flex-col gap-3 justify-center" : "justify-between gap-2 px-1"}`}>
        <div className="flex items-center gap-3 min-w-0">
          <div className={`grid size-9 shrink-0 place-items-center rounded-xl overflow-hidden ${isNanoBanana ? "bg-amber-500/10 border border-amber-500/40 shadow-[0_0_15px_rgba(234,179,8,0.25)]" : "bg-white/5 border border-white/10"}`}>
            {renderLogo(`size-5 ${isNanoBanana ? "text-amber-400" : "text-slate-200"}`)}
          </div>
          {!isCollapsed && (
            <div className="min-w-0">
              <div className="font-semibold tracking-tight text-slate-100 truncate">{tenantName}</div>
              <div className="text-[10px] text-slate-500 uppercase tracking-wider truncate">OS · {outletName}</div>
            </div>
          )}
        </div>
        <button 
          onClick={() => setIsCollapsed(!isCollapsed)} 
          className="rounded-lg border border-white/10 bg-white/5 p-2 text-slate-400 hover:bg-white/10 hover:text-white transition-all shrink-0 shadow-sm"
      ...
      <nav className="flex-1 space-y-1 overflow-x-hidden overflow-y-auto pr-1">
        {visibleNav.map(({ title, Icon, items }) => (
          <div key={title} className="mb-2">
            <button
              type="button"
              onClick={() => { if (isCollapsed) setIsCollapsed(false); setOpen(o => o.includes(title) ? o.filter(x => x !== title) : [...o, title]); }}
            ...
            >
              ...
              {!isCollapsed && (open.includes(title) ? <ChevronDown className="size-3 opacity-50" /> : <ChevronRight className="size-3 opacity-50" />)}
            </button>
      ...
      {(() => {
        ...
        const nameIsRole = rawName.toLowerCase() === activeRole
                        || rawName.toLowerCase() === roleLabel.toLowerCase();
        ...
        return (
          <div className={`mt-4 rounded-xl border p-2.5 flex items-center ${isNanoBanana ? "border-amber-500/30 bg-amber-500/[0.04] shadow-[0_0_20px_rgba(234,179,8,0.1)]" : "border-white/5 bg-white/[0.02]"} ${isCollapsed ? "flex-col justify-center gap-2" : "justify-between gap-2"}`}>
            <div className="flex items-center gap-2.5 min-w-0 flex-1">
              {/* Role-colored avatar */}
              <div className={`grid size-8 shrink-0 place-items-center rounded-full font-bold text-xs uppercase ${ROLE_AVATAR[activeRole] ?? ROLE_AVATAR.kasir}`}>
                {displayName ? displayName.charAt(0) : roleLabel.charAt(0)}
              </div>
              {!isCollapsed && (
                <div className="min-w-0 flex-1 flex items-center gap-1.5">
                  {displayName && (
                    <span className="text-[11px] font-semibold text-slate-200 truncate leading-none" title={displayName}>
                      {displayName}
                    </span>
                  )}
                  <span className={`shrink-0 text-[9px] font-bold px-1.5 py-0.5 rounded-full tracking-wider uppercase leading-none ${ROLE_BADGE[activeRole] ?? ROLE_BADGE.kasir}`}>
                    {roleLabel}
                  </span>
                </div>
              )}
            </div>
            <Link href="/" className="shrink-0 p-1.5 text-slate-500 hover:text-white hover:bg-white/10 rounded-lg transition-colors" title="Kembali ke Website">
