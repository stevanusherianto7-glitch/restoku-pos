...
export default function OwnerLayout({ children }: { children: ReactNode }) {
    ...
    return (
        <div
            className={`flex h-screen w-full text-slate-200 overflow-hidden font-sans transition-all duration-500 ${isNanoBanana ? 'bg-[#030712] selection:bg-amber-500/30 dark nano-banana' : 'bg-[#030b06] selection:bg-emerald-500/20 dark'}`}
        >
            {/* Sidebar */}
            <aside
                className={`flex h-full w-64 shrink-0 flex-col border-r p-4 transition-all duration-500 ${isNanoBanana ? 'border-amber-500/20 bg-[#030712]/95 backdrop-blur-3xl shadow-[0_0_50px_0_rgba(234,179,8,0.1)]' : 'border-emerald-900/30 bg-[#020905]'}`}
            >
                {/* Logo */}
                <div className="px-2 mb-8">
                    <TenantBrandLockup collapsed={false} />
                </div>
                ...
                <nav className="flex-1 space-y-1">
                    {ownerNav.map(({ name, href, Icon }) => (
                        ...
                        >
                            <Icon
                                className={`size-4 shrink-0 transition-colors ${isNanoBanana ? 'group-hover:text-amber-400' : 'group-hover:text-emerald-400'}`}
                            />
                ...
                {/* Bottom user card */}
                <div
                    className={`mt-4 rounded-xl border p-3 ${isNanoBanana ? 'border-amber-500/30 bg-amber-500/[0.04] shadow-[0_0_20px_rgba(234,179,8,0.1)]' : 'border-emerald-900/30 bg-emerald-900/10'}`}
                >
                    <div className="flex items-center gap-3 mb-3">
                        <div
                            className={`grid size-8 place-items-center rounded-full font-medium text-xs ${isNanoBanana ? 'bg-amber-500/20 text-amber-300 border border-amber-500/40' : 'bg-emerald-500/20 text-emerald-300'}`}
                        >
                            O
                        </div>
