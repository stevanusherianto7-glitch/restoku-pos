...
import { DollarSign, TrendingUp, Users, Award, AlertCircle, PieChart, Activity, Wallet, Receipt, CreditCard, ShoppingCart } from 'lucide-react';
...
export default function OwnerDashboard({ metrics, leaderboard, menuPerformance, filters }: Props) {
    const kpis = [
        ...
        { label: "Total Pendapatan", value: formatRp(metrics.total_revenue), icon: DollarSign, trend: metrics.revenue_growth, color: "text-amber-400", bg: "bg-amber-400/10", border: "border-amber-400/20" },
        { label: "Total Transaksi", value: metrics.total_transactions.toString(), icon: Activity, trend: "+5.2%", color: "text-blue-400", bg: "bg-blue-400/10", border: "border-blue-400/20" },
        { label: "Rata-rata Order", value: formatRp(metrics.average_order_value), icon: PieChart, trend: "+1.5%", color: "text-emerald-400", bg: "bg-emerald-400/10", border: "border-emerald-400/20" },
        { label: "Total Outlet Aktif", value: leaderboard.length.toString(), icon: Users, trend: "Tetap", color: "text-purple-400", bg: "bg-purple-400/10", border: "border-purple-400/20" },
    ];

    // Mock Data for Charts
    const dailyChart = [
        40, 60, 45, 80, 50, 90, 75, 40, 60, 45, 80, 50, 90, 75,
        40, 60, 45, 80, 50, 90, 75, 40, 60, 45, 80, 50, 90, 75, 100, 65
    ];
    ...
    return (
        <MainLayout>
            ...
            <div className="flex justify-between items-center mb-8 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
                ...
                <div className="flex gap-2">
                    <select className="bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm font-medium text-slate-200 focus:border-blue-500/50 outline-none backdrop-blur-md transition-colors hover:bg-white/10">
                        <option value="today" className="bg-slate-900">Hari Ini</option>
                        <option value="week" className="bg-slate-900">Minggu Ini</option>
                        <option value="month" className="bg-slate-900">Bulan Ini</option>
                    </select>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                {kpis.map((kpi, idx) => (
                    <Glass key={idx} className={`p-5 flex items-center gap-4 hover:-translate-y-1 hover:shadow-2xl transition-all duration-300 border-t-2 ${kpi.border}`}>
                        ...
                        <div>
                            ...
                            <div className="flex items-baseline gap-2">
                                ...
                                <span className={`text-xs font-semibold ${kpi.trend?.includes('-') ? 'text-red-400' : 'text-emerald-400'}`}>{kpi.trend}</span>
                            </div>
                        </div>
                    </Glass>
                ))}
            ...
            <Glass className="p-6 mb-6">
                ...
                <div className="h-48 w-full flex items-end gap-1.5 mt-4">
                    {dailyChart.map((val, i) => (
                        <div key={i} className="flex-1 bg-white/5 hover:bg-blue-500/30 transition-colors rounded-t-sm relative group cursor-pointer" style={{ height: `${val}%` }}>
                        ...
                        <div className="space-y-3">
                            {costVsRevenue.map((item, i) => (
                                <div key={i} className="flex items-center gap-3">
                                    ...
                                    <div className="w-10 text-right text-xs text-white font-medium">{item.percentage}%</div>
                                </div>
                            ))}
                        </div>
                    </Glass>
                            ...
                            <tbody className="divide-y divide-white/5">
                                {leaderboard.map((outlet, index) => (
                                    <tr key={outlet.id} className="group hover:bg-white/[0.02] transition-colors">
                                        ...
                                        <td className="py-4 text-emerald-400 font-medium">{formatRp(outlet.revenue)}</td>
                                        <td className="py-4">
                                            <span className={`inline-flex px-2 py-1 rounded-md text-xs font-medium border ${
                                                outlet.food_cost_percentage <= 35 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 
                                                outlet.food_cost_percentage <= 45 ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : 
                ...
                <Glass className="p-6">
                    ...
                    <div className="space-y-4">
                        {menuPerformance.top_performers.map((item, i) => (
                            <div key={i} className="flex justify-between items-center group p-3 rounded-xl hover:bg-white/5 transition-colors border border-transparent hover:border-white/10">
