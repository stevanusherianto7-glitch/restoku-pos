            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <Glass className="p-6">
                    ...
                    <div className="mb-8">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h4 className="font-semibold text-slate-200 uppercase tracking-widest text-xs">Pendapatan</h4>
                        </div>
                            ...
                            {revenueBreakdown.map((item, i) => (
                                <div key={i} className="flex justify-between items-center text-sm">
                                    ...
                                    <div className="flex items-center gap-4">
                                        <span className="text-white font-medium w-24 text-right">{formatRp(item.amount)}</span>
                                        <span className="text-slate-500 text-xs w-8 text-right">{item.percentage}%</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                        <div className="flex justify-between items-center pl-6 py-3 border-t border-white/10 mt-2">
                            ...
                            <div className="flex items-center gap-4">
                                <span className="font-bold text-emerald-400 w-24 text-right">{formatRp(totalRevenue)}</span>
                    ...
                    <div className="flex justify-between items-center p-5 bg-gradient-to-br from-emerald-500/20 to-emerald-600/10 border border-emerald-500/30 text-white rounded-xl shadow-[0_0_30px_rgba(16,185,129,0.15)] relative overflow-hidden">
                        ...
                        <div className="flex items-baseline gap-3">
                            <span className="text-3xl font-extrabold text-emerald-400">{formatRp(financials.net_profit)}</span>
                    ...
                    <div className="mt-8">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h4 className="font-semibold text-slate-200 uppercase tracking-widest text-xs">Pajak & Titipan</h4>
                        </div>
                ...
                <Glass className="p-6 flex flex-col">
                    ...
                    <div className="flex gap-4 mb-8">
                        <div className="flex-1 p-5 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 relative overflow-hidden group">
                            ...
                            <div className="flex items-center gap-2 text-emerald-400 mb-3 relative z-10">
                                <div className="p-1.5 bg-emerald-500/20 rounded-lg"><ArrowDownRight className="size-4" /></div>
                                <span className="font-medium text-sm">Kas Masuk</span>
                            </div>
                            <div className="text-2xl font-bold text-white relative z-10">{formatRp(financials.cash_in)}</div>
                        </div>
                        ...
                        <div className="flex-1 p-5 rounded-2xl border border-red-500/20 bg-red-500/5 relative overflow-hidden group">
                            ...
                            <div className="flex items-center gap-2 text-red-400 mb-3 relative z-10">
                                <div className="p-1.5 bg-red-500/20 rounded-lg"><ArrowUpRight className="size-4" /></div>
                                <span className="font-medium text-sm">Kas Keluar</span>
                            </div>
                            <div className="text-2xl font-bold text-white relative z-10">{formatRp(financials.cash_out)}</div>
                        </div>
                    </div>
                    ...
                    <div className="p-6 rounded-2xl bg-black/40 border border-white/5 text-white flex-1 flex flex-col justify-center text-center">
                        <p className="text-slate-400 text-sm font-medium mb-2 uppercase tracking-widest">Net Cash Flow</p>
                        <h3 className={`text-5xl font-black mb-6 tracking-tight ${financials.cash_in - financials.cash_out >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                        ...
                        <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl text-left">
                            <p className="text-[11px] text-blue-200/70 leading-relaxed">
                                <strong className="text-blue-300">Catatan Owner:</strong> Arus kas (Cash Flow) berbeda dengan Laba Bersih (Net Profit) karena memperhitungkan perpindahan uang tunai aktual. Saldo bisa lebih rendah akibat keterlambatan pencairan dana QRIS/GoFood, atau lebih tinggi karena uang muka pesanan bulan depan.
                            </p>
                        </div>
                    </div>
                </Glass>
