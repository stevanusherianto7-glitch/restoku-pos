...
import { ChefHat, Search, MessageCircle, ShoppingBag, Plus, Minus, Check, MapPin, Sparkles, ShoppingCart, HelpCircle, CalendarDays, Image as ImageIcon, Users, Phone, Clock3, Calendar, CheckCircle2 } from "lucide-react";
...
export default function CustomerView() {
  ...
  const theme = {
    premium: {
      ...
      cardHover: "bg-white/[0.03] border border-white/5 hover:border-white/10 p-4 rounded-3xl shadow-sm flex gap-4 transition-all group relative overflow-hidden",
      ...
      inputLarge: "w-full h-20 rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-xs text-slate-200 outline-none focus:border-emerald-500/40 focus:ring-1 focus:ring-emerald-500/25 transition-all resize-none",
      btnPrimary: "w-full py-3.5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-2 mt-4 shadow-lg shadow-emerald-500/10 disabled:opacity-50",
      ...
      bannerBg: "relative px-5 py-6 bg-gradient-to-r from-emerald-950/40 via-teal-950/20 to-slate-950/40 border-b border-white/5 overflow-hidden",
      categoryBtn: (active: boolean) => active ? "bg-emerald-500 border-emerald-400 text-slate-950 shadow-lg shadow-emerald-500/10 scale-95" : "bg-white/5 border-white/10 text-slate-300 hover:border-white/20",
    ...
    minimalist: {
      ...
      cardHover: "bg-white border border-slate-200 p-4 rounded-2xl shadow-sm flex gap-4 transition-all group relative overflow-hidden",
      ...
      inputLarge: "w-full h-20 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs text-slate-800 outline-none focus:border-slate-400 focus:ring-1 focus:ring-slate-400/25 transition-all resize-none",
      btnPrimary: "w-full py-3.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-black transition-all flex items-center justify-center gap-2 mt-4 shadow-md disabled:opacity-50",
      ...
      categoryBtn: (active: boolean) => active ? "bg-slate-900 border-slate-900 text-white shadow-sm scale-95" : "bg-white border-slate-200 text-slate-600 hover:bg-slate-50",
      categoryList: "flex gap-2 overflow-x-auto px-4 py-3 sticky top-[77px] z-30 bg-slate-50/90 backdrop-blur-md",
    },
    cozy: {
      outer: "min-h-screen w-full bg-[#fcf8f2] text-amber-950 flex flex-col font-sans max-w-md mx-auto shadow-2xl relative border-x border-amber-900/10",
      ...
      cardHover: "bg-white/80 border border-amber-900/10 p-4 rounded-2xl shadow-sm flex gap-4 transition-all group relative overflow-hidden",
      ...
      inputLarge: "w-full h-20 rounded-xl border border-amber-900/15 bg-white px-4 py-2.5 text-xs text-amber-950 outline-none focus:border-amber-600/40 focus:ring-1 focus:ring-amber-600/25 transition-all resize-none",
      btnPrimary: "w-full py-3.5 bg-amber-850 hover:bg-amber-800 text-white rounded-xl text-xs font-black transition-all flex items-center justify-center gap-2 mt-4 shadow-md disabled:opacity-50",
      ...
      categoryBtn: (active: boolean) => active ? "bg-amber-850 border-amber-800 text-white shadow-sm scale-95" : "bg-white border-amber-900/10 text-amber-800 hover:bg-[#faf4ec]",
    ...
    nanoBanana: {
      ...
      cardHover: "bg-gradient-to-br from-amber-500/[0.05] to-transparent border border-amber-500/20 hover:border-amber-500/50 p-4 rounded-3xl shadow-[0_4px_20px_rgba(234,179,8,0.08)] flex gap-4 transition-all group relative overflow-hidden",
      ...
      inputLarge: "w-full h-20 rounded-xl border border-amber-500/30 bg-[#030712]/60 px-4 py-2.5 text-xs text-amber-100 outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400/30 transition-all resize-none",
      btnPrimary: "w-full py-3.5 bg-gradient-to-r from-yellow-400 via-amber-500 to-yellow-500 hover:brightness-110 text-slate-950 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-2 mt-4 shadow-[0_0_20px_rgba(234,179,8,0.4)] disabled:opacity-50",
      ...
      bannerBg: "relative px-5 py-6 bg-gradient-to-r from-amber-950/40 via-yellow-950/20 to-[#030712] border-b border-amber-500/20 overflow-hidden",
      categoryBtn: (active: boolean) => active ? "bg-gradient-to-r from-yellow-400 via-amber-500 to-yellow-500 border-amber-400 text-slate-950 font-extrabold shadow-[0_0_15px_rgba(234,179,8,0.4)] scale-95" : "bg-amber-500/[0.05] border-amber-500/20 text-amber-200/80 hover:border-amber-500/40",
      categoryList: "flex gap-2 overflow-x-auto px-4 py-3 sticky top-[77px] z-30 bg-[#030712]/90 backdrop-blur-xl border-b border-amber-500/10",
    }
  };
  ...
  const handleCheckout = async () => {
    ...
    const itemsList = Object.entries(cart).map(([idStr, qty]) => {
      const item = MOCK_ITEMS.find(i => i.id === parseInt(idStr));
    ...
    try {
      const displayTable = orderType === "take_away" 
    ...
    <div className={activeTheme.outer}>
      ...
      {!isOutletOpen && (
        <div className="bg-rose-500/20 border-b border-rose-500/30 px-4 py-3 flex items-center gap-3 text-rose-300">
          ...
          <div className="text-xs">
            ...
            <p className="text-[11px] opacity-90">{outletScheduleMsg || "Restoran sedang di luar jam operasional."}</p>
          </div>
        </div>
      )}
        ...
        <div className="flex bg-white/5 border border-white/10 rounded-xl p-0.5 max-w-full overflow-x-auto gap-0.5">
          <button 
            ...
            className={`px-2.5 py-1.5 rounded-lg text-[10px] font-bold transition-all shrink-0 ${
              activeTab === "menu" ? (isNanoBanana ? "bg-amber-500 text-slate-950 font-bold shadow-[0_0_10px_rgba(234,179,8,0.3)]" : "bg-emerald-500 text-slate-950 shadow") : "text-slate-400"
            }`}
          >
            Menu
          </button>
          <button 
            ...
            className={`px-2.5 py-1.5 rounded-lg text-[10px] font-bold transition-all shrink-0 ${
              activeTab === "reservasi" ? (isNanoBanana ? "bg-amber-500 text-slate-950 font-bold shadow-[0_0_10px_rgba(234,179,8,0.3)]" : "bg-emerald-500 text-slate-950 shadow") : "text-slate-400"
            }`}
          >
            Reservasi
          </button>
          <button 
            ...
            className={`px-2.5 py-1.5 rounded-lg text-[10px] font-bold transition-all shrink-0 ${
              activeTab === "galeri" ? (isNanoBanana ? "bg-amber-500 text-slate-950 font-bold shadow-[0_0_10px_rgba(234,179,8,0.3)]" : "bg-emerald-500 text-slate-950 shadow") : "text-slate-400"
            }`}
          >
            Galeri
          </button>
          <button 
            ...
            className={`px-2.5 py-1.5 rounded-lg text-[10px] font-bold transition-all flex items-center gap-1 shrink-0 ${
              activeTab === "cart" ? (isNanoBanana ? "bg-amber-500 text-slate-950 font-bold shadow-[0_0_10px_rgba(234,179,8,0.3)]" : "bg-emerald-500 text-slate-950 shadow") : "text-slate-400"
            }`}
          >
      ...
      {activeTab === "menu" && (<>
          ...
          <section className={activeTheme.bannerBg}>
            <div className={`absolute -top-12 -right-12 size-36 rounded-full blur-3xl ${isNanoBanana ? "bg-amber-500/20" : "bg-emerald-500/10"}`} />
            <div className="relative z-10">
              <span className={activeTheme.badge}>
                <Sparkles className={`size-3 ${isNanoBanana ? "text-amber-300" : "text-emerald-400"}`} /> {isNanoBanana ? "Cyber Gold Executive Dining" : "Premium Dining Experience"}
              </span>
              <h2 className="text-xl font-extrabold text-white tracking-tight leading-tight">Sajian Cita Rasa Terbaik</h2>
              <p className="text-xs text-slate-400 mt-1 leading-relaxed">Pesan hidangan otentik langsung ke dapur dari genggaman Anda.</p>
            </div>
          </section>
          ...
          <main className="flex-1 px-4 pb-28 space-y-4 overflow-y-auto">
            {filteredItems.length > 0 ? (
              filteredItems.map(item => (
                <div key={item.id} className="flex gap-4 bg-white/[0.03] p-4 rounded-3xl shadow-sm border border-white/5 hover:border-white/10 transition-all group relative overflow-hidden">
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-between">
                    <div>
                      <div className="flex items-start justify-between gap-1.5">
                        <h3 className={`text-sm font-bold text-white leading-snug transition-colors truncate ${isNanoBanana ? "group-hover:text-amber-400" : "group-hover:text-emerald-400"}`}>{item.name}</h3>
                      </div>
                      <p className="text-[10px] text-slate-400 leading-relaxed mt-1 line-clamp-2">{item.description || "Hidangan lezat diolah higienis dengan resep rahasia."}</p>
                    </div>
                    <div className="flex items-center justify-between mt-2 pt-2 border-t border-white/5">
                      <span className={`text-xs font-mono font-bold ${isNanoBanana ? "text-amber-400" : "text-emerald-400"}`}>{formatRupiah(item.price)}</span>
                      <div className="flex items-center">
                        {cart[item.id] ? (
                          <div className="flex items-center bg-white/5 border border-white/10 rounded-full p-0.5">
                            ...
                            <span className="w-8 text-center text-xs font-bold text-white">{cart[item.id]}</span>
                            <button 
            ...
            {cartTotalItems > 0 ? (
              <div className="space-y-3 max-h-[calc(100vh-320px)] overflow-y-auto pr-1">
                ...
                {Object.entries(cart).map(([idStr, qty]) => {
                  ...
                  if (!item) return null;
                  return (
                    <div key={id} className="flex items-center gap-3 bg-white/[0.02] p-3 rounded-2xl border border-white/5">
                      <ProductImage src={item.image} alt={item.name} variant="small" className="size-12 rounded-xl object-cover" />
                      <div className="flex-1 min-w-0">
                        <h4 className="text-xs font-bold text-white truncate">{item.name}</h4>
                        <p className="text-[10px] text-emerald-400 font-mono mt-0.5">{formatRupiah(item.price)}</p>
                      </div>
                      <div className="flex items-center bg-white/5 border border-white/10 rounded-full p-0.5">
                        <button onClick={() => removeFromCart(id)} className="size-5 rounded-full bg-emerald-500 text-slate-950 flex items-center justify-center font-bold"><Minus className="size-2.5" /></button>
                        <span className="w-6 text-center text-xs font-bold text-white">{qty}</span>
                        <button onClick={() => addToCart(id)} className="size-5 rounded-full bg-emerald-500 text-slate-950 flex items-center justify-center font-bold"><Plus className="size-2.5" /></button>
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="py-24 text-center text-slate-500">
                ...
                <p className="text-xs text-slate-500 mt-1">Kembali ke tab menu untuk memilih hidangan terbaik kami.</p>
                <button 
                  onClick={() => setActiveTab("menu")}
              ...
              <div className="py-12 px-4 text-center space-y-4 bg-emerald-950/10 border border-emerald-500/20 rounded-3xl animate-in fade-in duration-300">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-400 leading-relaxed">
                    Terima kasih {rName}. Pengajuan reservasi Anda pada {rDate} pukul {rTime} sedang ditinjau oleh staf kami.
                  </p>
                </div>
                <button
                  onClick={() => {
      ...
      {activeTab === "galeri" && (
        <main className="flex-1 p-5 pb-28 flex flex-col justify-between overflow-y-auto space-y-6">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { title: "Ulang Tahun", desc: "Momen manis berkesan", img: "https://images.unsplash.com/photo-1530103862676-de8c9debad1d?w=500" },
                { title: "Intimate Wedding", desc: "Suasana sakral premium", img: "https://images.unsplash.com/photo-1519741497674-611481863552?w=500" },
                { title: "Corporate Meeting", desc: "Fasilitas meeting lengkap", img: "https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?w=500" },
                { title: "Family Gathering", desc: "Kebersamaan tak terlupakan", img: "https://images.unsplash.com/photo-1528605248644-14dd04022da1?w=500" },
              ].map((item, idx) => (
                <div key={idx} className="group relative rounded-2xl overflow-hidden border border-white/5 bg-white/[0.02]">
                  <div className="h-28 w-full overflow-hidden bg-slate-950">
                    <img src={item.img} alt={item.title} className="size-full object-cover opacity-80 group-hover:scale-105 transition-all duration-300" />
                  </div>
                  <div className="p-3">
        ...
        <div className="absolute bottom-20 inset-x-4 z-40 bg-slate-900/60 backdrop-blur-md p-4 rounded-3xl border border-white/10 flex items-center justify-between shadow-2xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[9px] text-slate-400 leading-tight">Dapatkan updates & status pesanan realtime.</p>
            </div>
          </div>
          <button 
      ...
      {cartTotalItems > 0 && !orderSuccess && (
        <div className="absolute bottom-5 inset-x-4 z-40">
          ...
          >
            <span className="bg-slate-950/20 px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider text-slate-900">{cartTotalItems} Item</span>
      ...
      {orderSuccess && (
        <div className="absolute inset-0 bg-black/90 backdrop-blur-md z-50 flex items-center justify-center p-6">
          <div className="bg-slate-900/90 border border-white/10 rounded-3xl p-6 w-full max-w-xs flex flex-col items-center text-center shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <div className={`size-16 rounded-full flex items-center justify-center mb-4 shadow-lg transition-all ${
              orderTone === 'amber' ? 'bg-amber-500/10 border-amber-500/20 text-amber-400 shadow-amber-500/5' :
              orderTone === 'blue' ? 'bg-blue-500/10 border-blue-500/20 text-blue-400 shadow-blue-500/5' :
            ...
            <h3 className="text-base font-extrabold text-white mb-1">
              {orderStatus === 'Antrian Masuk' ? 'Menunggu Konfirmasi' :
               orderStatus === 'Sedang Dimasak' ? 'Sedang Dimasak' :
               orderStatus === 'Siap Sajikan' ? 'Pesanan Siap Sajikan!' :
               'Pesanan Selesai!'}
            </h3>
            <p className="text-xs text-slate-400 leading-relaxed mb-4">
              {orderStatus === 'Antrian Masuk' ? 'Dapur sedang meninjau pesanan Anda.' :
               orderStatus === 'Sedang Dimasak' ? 'Koki sedang meracik hidangan lezat Anda.' :
               orderStatus === 'Siap Sajikan' ? 'Pelayan kami sedang mengantarkan hidangan ke meja Anda.' :
            ...
            <div className="w-full bg-white/[0.02] border border-white/5 p-4 rounded-2xl mb-4 text-left space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-[9px] uppercase font-bold text-slate-500 tracking-wider">Status Pesanan</span>
                <span className={`text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded ${
                  orderTone === 'amber' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' :
                  orderTone === 'blue' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' :
              ...
              <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden flex">
                <div className={`h-full transition-all duration-500 ${
                  orderStatus === 'Antrian Masuk' ? 'w-1/3 bg-amber-500' :
                  orderStatus === 'Sedang Dimasak' ? 'w-2/3 bg-blue-500' :
              ...
              <div className="flex justify-between text-[8px] font-bold text-slate-500 uppercase tracking-wider">
                ...
                <span className={orderStatus === 'Siap Sajikan' || orderStatus === 'Selesai' ? 'text-emerald-400' : ''}>Saji</span>
              </div>
            </div>
            ...
            <div className="w-full bg-white/[0.02] border border-white/5 p-3 rounded-2xl mb-4 text-left">
              <div className="flex justify-between">
                <div>
                  <p className="text-[9px] uppercase font-bold text-slate-500 tracking-wider">Nomor Meja</p>
                  <p className="text-sm font-extrabold text-emerald-400 mt-0.5">{tableNumber ? `Meja ${tableNumber}` : "Meja 1"}</p>
                </div>
                <div>
                  <p className="text-[9px] uppercase font-bold text-slate-500 tracking-wider text-right">Order ID</p>
                  <p className="text-xs font-bold text-slate-300 mt-0.5 text-right">{activeOrderId}</p>
                </div>
              </div>
            </div>
            
            <button 
              onClick={() => { setOrderSuccess(false); setCart({}); setActiveTab("menu"); }} 
