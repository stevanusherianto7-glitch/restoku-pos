        ...
        <div className="space-y-5">
          <Glass className="p-5">
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1 rounded-lg border border-white/10 bg-white/5 px-4 py-2.5 flex items-center justify-between">
                ...
                <button className="text-slate-400 hover:text-white transition-colors" title="Salin Tautan"><Copy className="size-4" /></button>
              </div>
              <button className="rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-4 py-2.5 text-sm font-medium hover:bg-emerald-500/20 transition-colors">Buka Preview</button>
            </div>
            <div className="mt-5 grid grid-cols-2 gap-4">
              <div className="rounded-xl border border-white/5 bg-white/[0.02] p-4 flex items-center gap-4">
                ...
                <div>
                  ...
                  <button className="text-xs text-blue-400 hover:text-blue-300 mt-0.5">Unduh Semua QR</button>
                </div>
              </div>
              <div className="rounded-xl border border-white/5 bg-white/[0.02] p-4 flex items-center gap-4">
          ...
          <Glass className="p-5">
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-2">Tema Warna Utama</label>
                <div className="flex gap-3">
                  {["bg-blue-500", "bg-emerald-500", "bg-red-500", "bg-amber-500", "bg-violet-500", "bg-slate-900"].map((color, i) => (
                    <button key={i} className={`size-8 rounded-full ${color} ${i === 0 ? "ring-2 ring-white ring-offset-2 ring-offset-[#09090b]" : "opacity-70 hover:opacity-100"} transition-all shadow-sm`} />
                  ))}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-5">
                {([{ label: "Logo Restoran", hint: "Upload (1:1)", Icon: ImageIcon }, { label: "Banner / Cover", hint: "Upload (16:9)", Icon: Upload }] as Array<{ label: string; hint: string; Icon: ElementType }>).map(({ label, hint, Icon }) => (
                  <div key={label}>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-2">Pesan Sambutan</label>
                <textarea className="w-full h-20 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-200 outline-none focus:border-white/20 transition-colors resize-none"
            ...
            <div className="mt-6 pt-5 border-t border-white/5 flex justify-end">
              <button className="rounded-lg bg-slate-100 hover:bg-white px-5 py-2.5 text-sm font-medium text-slate-900 transition-colors">Simpan Perubahan</button>
            </div>
          </Glass>
        </div>
          ...
          <div className="relative w-[300px] h-[600px] rounded-[2.5rem] border-8 border-slate-900 bg-[#f8fafc] shadow-2xl overflow-hidden ring-1 ring-white/10">
            ...
            <div className="h-32 bg-gradient-to-br from-slate-200 to-slate-300 relative">
              ...
              <div className="absolute -bottom-6 left-5 size-14 rounded-full bg-white p-1 shadow-md">
                <div className="size-full rounded-full bg-blue-500 flex items-center justify-center text-white font-bold">R</div>
              </div>
            </div>
            <div className="pt-8 px-5 pb-4 bg-white shadow-sm relative z-10">
              ...
              <p className="text-xs text-slate-500 mt-1 leading-relaxed">Selamat datang di Restoku Senopati.</p>
            </div>
            <div className="flex gap-2 overflow-x-auto px-5 py-3 border-b border-slate-100 bg-[#f8fafc]">
              <div className="rounded-full bg-blue-500 px-3 py-1 text-[11px] font-medium text-white shadow-sm whitespace-nowrap">Makanan</div>
              <div className="rounded-full bg-white border border-slate-200 px-3 py-1 text-[11px] font-medium text-slate-600 whitespace-nowrap">Minuman</div>
              <div className="rounded-full bg-white border border-slate-200 px-3 py-1 text-[11px] font-medium text-slate-600 whitespace-nowrap">Pelengkap</div>
            </div>
            <div className="p-4 space-y-3 bg-[#f8fafc] h-full overflow-y-auto pb-32">
              {["Nasi Goreng Spesial", "Ayam Geprek", "Es Kopi Susu"].map((name, i) => (
                <div key={i} className="flex gap-3 bg-white p-3 rounded-xl shadow-sm border border-slate-100">
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-center">
                    ...
                    <p className="text-xs font-medium text-blue-600 mt-1">{formatRupiah([25000, 28000, 18000][i])}</p>
                  </div>
                  <div className="flex items-end">
                    <button className="rounded-full bg-slate-50 size-7 flex items-center justify-center text-blue-600 font-medium shadow-sm border border-slate-100">+</button>
                  </div>
                </div>
              ))}
            </div>
