    ...
    return (
      <MainLayout>
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center gap-6">
          ...
          <Link href="/dashboard" className="px-6 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-slate-200 text-sm font-semibold transition-colors">
...
function KDSContent() {
  ...
  return (
    <MainLayout>
      ...
      <Screen title="Kitchen Display System" action={
        <button 
        ...
        {getAggregatedItems().length > 0 && (
          <div className="mb-6 bg-slate-950/40 border border-white/5 rounded-3xl p-5 flex items-center justify-between font-mono">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-black text-slate-200 uppercase tracking-wider">PANDUAN MASAK SEKALIGUS</h3>
                <p className="text-xs text-slate-500 mt-0.5">Total porsi menu yang sama di seluruh antrean aktif.</p>
              </div>
            </div>
            <div className="flex flex-wrap gap-2 justify-end">
              {getAggregatedItems().map(([name, qty]) => (
                <div key={name} className="flex items-center gap-2.5 bg-white/5 border border-white/10 px-4 py-2 rounded-2xl">
                  ...
                  {orders.map((o) => (
                    <div key={o.id} className={`rounded-2xl border p-5 ${cardToneMap[tone]}`}>
                      <div className="mb-4 flex justify-between items-start">
                        <div>
                          <b className="text-6xl font-bold tracking-tight block text-slate-100">{o.table}</b>
                          ...
                          <div className="mt-3">
                            {o.table.toLowerCase().includes("takeaway") ? (
                              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-orange-500/10 border border-orange-500/20 text-orange-400 text-xs font-black uppercase tracking-wider">
                                🛍️ TAKE AWAY <span className="text-[10px] font-normal lowercase text-slate-400">(Siapkan Box & Paperbag)</span>
                              </span>
                            ) : (
                        ...
                        <span className="font-mono text-4xl opacity-90 flex items-center gap-2 font-bold bg-black/20 px-3 py-2 rounded-xl">
                          <Clock3 className="size-8" />{o.time}'
                        </span>
                      </div>
                      <div className="space-y-2 text-3xl font-medium mt-6 border-t border-white/10 pt-4">
                        {o.items.map((item, i) => {
                          ...
                          return (
                            <p key={i} className={isNote ? "text-2xl opacity-70 font-normal italic pl-4" : "text-slate-200"}>
                              {item}
                            </p>
                          );
                        })}
