...
import { Screen, Glass, Badge, formatRupiah, toneMap, cardToneMap, PlanBadge, TAX_LABELS, useTenantSettings } from "../../Components/Shared";
import { useSubscription } from "../../Hooks/useSubscription";
import { Store, MapPin, Receipt, Clock, LocateFixed, Percent, AlertTriangle, ChefHat, Coffee, Wine, Flame, Cake, Pizza, Image as ImageIcon, Sparkles, Plus, Trash2, Edit2, Check, X } from "lucide-react";
...
function PengaturanOutletInner() {
  ...
  const { 
    tenantName, tenantLogo, tenantImage, saveSettings, tenantLayout, saveLayout,
    staffOwner, employees, saveEmployees
  } = useTenantSettings();
  ...
  useEffect(() => {
    if (tenant) {
      ...
      const sNpwp = localStorage.getItem("outlet_npwp"); if (sNpwp) setNpwpInput(sNpwp);
      const sNib = localStorage.getItem("outlet_nib"); if (sNib) setNibInput(sNib);
      const sAlamat = localStorage.getItem("outlet_alamat"); if (sAlamat) setAlamatInput(sAlamat);
      const sTelepon = localStorage.getItem("outlet_telepon"); if (sTelepon) setTeleponInput(sTelepon);
      const sTaxType = localStorage.getItem("outlet_tax_type"); if (sTaxType) setTaxType(sTaxType as "pbjt" | "ppn");
      const sTaxRate = localStorage.getItem("outlet_tax_rate"); if (sTaxRate) setTaxRateInput(Number(sTaxRate));
      const sSvcCharge = localStorage.getItem("outlet_service_charge"); if (sSvcCharge) setServiceChargeInput(Number(sSvcCharge));
    }
    ...
    if (outlet) {
      ...
      if (outlet.operating_hours && typeof outlet.operating_hours === "object") {
        const dayMap: Record<string, string> = { mon: "Senin", tue: "Selasa", wed: "Rabu", thu: "Kamis", fri: "Jumat", sat: "Sabtu", sun: "Minggu" };
      ...
      const sNama = localStorage.getItem("outlet_nama"); if (sNama) setNamaOutletInput(sNama);
      const sLat = localStorage.getItem("outlet_latitude"); if (sLat) setLatitudeInput(sLat);
      const sLng = localStorage.getItem("outlet_longitude"); if (sLng) setLongitudeInput(sLng);
      const sJam = localStorage.getItem("outlet_jam_operasional");
      if (sJam) { try { setJamOperasional(JSON.parse(sJam)); } catch {} }
    }
    ...
    const sMode = localStorage.getItem("outlet_screen_mode"); if (sMode === "terang" || sMode === "gelap" || sMode === "glassmorphic" || sMode === "nano-banana") setScreenMode(sMode as any); else setScreenMode("nano-banana");
    const sTaxActive = localStorage.getItem("outlet_tax_active"); if (sTaxActive !== null) setIsTaxActive(sTaxActive === "true");
  ...
  const runLocalGeocoder = (address: string) => {
    ...
    if (addrUpper.includes("SENOPATI") || addrUpper.includes("KEBAYORAN") || addrUpper.includes("JAKARTA SELATAN")) {
    ...
    } else if (addrUpper.includes("JAKARTA PUSAT") || addrUpper.includes("MENTENG") || addrUpper.includes("SUDIRMAN")) {
    ...
    } else if (addrUpper.includes("BALI") || addrUpper.includes("KUTA") || addrUpper.includes("SEMINYAK") || addrUpper.includes("DENPASAR")) {
  ...
  const handleAddEmployee = () => {
    ...
    router.post('/api/outlet-settings/karyawan', {
  ...
  const handleSaveEdit = () => {
    ...
    router.put(`/api/outlet-settings/karyawan/${editingId}`, {
  ...
  const handleSaveAllChanges = () => {
    ...
    router.put('/api/outlet-settings/all', {
      profil: {
        name: nameInput,
      ...
      onError: (err: any) => {
        ...
        alert("⚠️ Pengaturan tersimpan secara lokal, namun gagal sync ke server database: " + JSON.stringify(err));
      }
    });
  };
  ...
  const optionBtnClass = (active: boolean) => `flex flex-col text-left rounded-xl border p-3 transition-all relative overflow-hidden group ${
    active
      ? isLight
      ...
      <div className="grid grid-cols-[240px_1fr] gap-5 items-start">
        <Glass className="p-3">
          <div className="space-y-1">
            {([{ label: "Profil Outlet", Icon: Store }, { label: "Lokasi Restoran", Icon: MapPin }, { label: "Pajak & Tarif", Icon: Percent }, { label: "Tampilan Struk", Icon: Receipt }, { label: "Jam Operasional", Icon: Clock }] as Array<{ label: string; Icon: ElementType }>).map(({ label, Icon }) => (
              <button 
                key={label} 
                type="button" 
        ...
        <div className="space-y-5">
          ...
          <div id="profil" className="space-y-5">
              ...
              <Glass className="p-6 border-emerald-500/10">
                <h2 className={h2Class}>
                  <Sparkles className="size-4 text-emerald-400" /> Branding Kustom Tenant (White-Label)
                </h2>
                <div className="space-y-5">
                  ...
                  <div className={`space-y-4 pt-4 border-t ${isLight ? "border-slate-200" : "border-white/5"}`}>
                        ...
                        <div key={emp.id} className={cardRowClass}>
                          {editingId === emp.id ? (
                            ...
                            <div className="flex items-center gap-3 flex-1">
                              <div className={`text-[9px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-md border shrink-0 w-20 text-center ${
                                emp.role === "cashier" || emp.role === "kasir" ? "bg-blue-500/10 border-blue-500/20 text-blue-500" :
                                emp.role === "kitchen" ? "bg-red-500/10 border-red-500/20 text-red-500" :
                                emp.role === "waiter" ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-500" :
                                "bg-amber-500/10 border-amber-500/20 text-amber-500"
                              }`}>
                                {emp.role}
                              </div>
                              <div className={`text-xs font-bold truncate ${isLight ? "text-slate-900" : "text-slate-200"}`}>{emp.name}</div>
                              <div className={`text-xs font-mono ml-auto px-2 py-0.5 rounded-md ${isLight ? "text-slate-600 bg-slate-200/60" : "text-slate-500 bg-white/5"}`}>Email: {emp.email || "-"}</div>
                            </div>
                          )}
                    ...
                    <div className={`border border-dashed p-3 rounded-xl space-y-3 ${isLight ? "bg-slate-50 border-slate-300" : "bg-white/[0.02] border-white/10"}`}>
                    ...
                    <div className={`flex gap-3 border p-3 rounded-xl ${isLight ? "bg-slate-50 border-slate-200" : "bg-white/[0.01] border-white/5"}`}>
                      <div className="flex-1 space-y-1">
                        <label className="text-[10px] font-bold text-purple-500 uppercase tracking-wide">Nama Owner</label>
                        <input 
                          value={ownerInput}
                      ...
                      <div className="w-24 space-y-1">
                        <label className="text-[10px] font-bold text-purple-500 uppercase tracking-wide">Akses Login</label>
              ...
              <Glass className="p-6">
                ...
                <div className="grid grid-cols-2 gap-5">
                  <div className="space-y-1.5">
                    <label className={labelClass}>Cabang Outlet</label>
                    <input value={namaOutletInput} onChange={e => setNamaOutletInput(e.target.value)} placeholder="e.g. Senopati / Cabang Senopati" className={inputClass} />
                  </div>
                  <div className="space-y-1.5">
                    <label className={labelClass}>Telepon</label>
                    <input value={teleponInput} onChange={e => setTeleponInput(e.target.value)} className={inputClass} />
                  </div>
                  <div className="space-y-1.5">
              ...
              <Glass className="p-6">
                ...
                <div className="space-y-4">
                  <p className={descClass}>
                    Pilih mode tampilan antarmuka (UI Screen) untuk dasbor admin, kasir POS, dan operasional outlet Anda.
                  </p>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {[
                      ...
                      >
                        <div className={`h-8 w-full rounded-lg bg-gradient-to-br ${opt.preview} border ${isLight ? "border-slate-200" : "border-white/5"} mb-2.5 flex items-center justify-center shadow-inner`}>
                          <span className={`text-[9px] font-extrabold opacity-80 ${isLight ? "text-slate-800" : "text-slate-200"}`}>UI Mode</span>
                        </div>
                        <div className={`font-bold text-[11px] ${isLight ? "text-slate-900" : "text-slate-200"}`}>{opt.name}</div>
                        <div className={`text-[9px] mt-0.5 leading-tight ${isLight ? "text-slate-600" : "text-slate-500"}`}>{opt.desc}</div>
                        
                        {screenMode === opt.id && (
                          <div className={`absolute top-1 right-1 size-2 rounded-full ${isLight ? "bg-blue-600" : "bg-amber-400"} animate-pulse`} />
                        )}
                      </button>
                    ))}
                  </div>
              ...
              <Glass className="p-6">
                ...
                <div className="space-y-4">
                  <p className={descClass}>
                    Pilih tema tampilan e-menu yang sesuai dengan konsep restoran Anda. Perubahan akan langsung terlihat pada menu digital tamu.
                  </p>
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      ...
                      >
                        ...
                        <div className={`h-8 w-full rounded-lg bg-gradient-to-br ${opt.preview} border ${isLight ? "border-slate-200" : "border-white/5"} mb-2.5 flex items-center justify-center`}>
                          <span className={`text-[8px] font-bold opacity-80 ${isLight ? "text-slate-800" : "text-slate-200"}`}>Aa</span>
                        </div>
                        <div className={`font-bold text-[11px] ${isLight ? "text-slate-900" : "text-slate-200"}`}>{opt.name}</div>
                        <div className={`text-[9px] mt-0.5 leading-tight ${isLight ? "text-slate-600" : "text-slate-500"}`}>{opt.desc}</div>
                        
                        {tenantLayout === opt.id && (
                          <div className={`absolute top-1 right-1 size-2 rounded-full ${isLight ? "bg-blue-600" : "bg-emerald-500"}`} />
                        )}
                      </button>
                    ))}
                  </div>
          ...
          <div id="lokasi" className="space-y-5">
            <Glass className="p-6">
              ...
              <div className="space-y-4">
                <p className={descClass}>
                  Tentukan koordinat GPS restoran untuk mencegah order fiktif dari luar area restoran (misalnya pesanan dine-in palsu dari jarak jauh).
                </p>
                <div className="grid grid-cols-2 gap-5">
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center">
                  ...
                  <div className="space-y-1.5 col-span-2">
                    ...
                    <p className={`text-[11px] ${isLight ? "text-slate-600" : "text-slate-500"}`}>Radius 50 meter disarankan untuk restoran di dalam mall.</p>
                  </div>
                </div>
                <div className="flex gap-3 pt-2">
                  ...
                  <button className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${isLight ? "bg-blue-600 hover:bg-blue-700 text-white shadow-xs" : "bg-slate-100 hover:bg-white text-slate-900"}`}>Simpan Lokasi</button>
                </div>
              </div>
            </Glass>
          </div>
          ...
          <div id="pajak" className="space-y-5">
            <Glass className="p-6">
              ...
              <div className={`mb-5 rounded-xl border p-4 space-y-2 ${isLight ? "border-emerald-300 bg-emerald-50" : "border-emerald-500/20 bg-emerald-500/5"}`}>
                <div className="flex items-center justify-between">
                  <span className={`text-xs font-bold flex items-center gap-1.5 ${isLight ? "text-emerald-800" : "text-emerald-400"}`}>
                    <Sparkles className="size-3.5 text-emerald-500" /> Referensi Regulasi Perpajakan Restoran (Indonesia)
                  </span>
                  <span className={`text-[10px] font-mono px-2 py-0.5 rounded-full border ${isLight ? "bg-emerald-200/80 text-emerald-900 border-emerald-400 font-bold" : "bg-emerald-500/20 text-emerald-300 border-emerald-500/30"}`}>UU HKPD No. 1/2022</span>
                </div>
                <p className={`text-[11px] leading-relaxed ${isLight ? "text-slate-700" : "text-slate-300"}`}>
                  Berdasarkan <strong className={isLight ? "text-slate-950" : "text-white"}>UU No. 1 Tahun 2022 (HKPD) Pasal 53 & 58</strong>, makanan dan/atau minuman yang disediakan oleh restoran, kafe, atau rumah makan merupakan objek <strong className={isLight ? "text-emerald-700 font-bold" : "text-emerald-300"}>PBJT (Pajak Barang dan Jasa Tertentu)</strong> yang dipungut Pemerintah Daerah dengan tarif maksimal <strong className={isLight ? "text-slate-950" : "text-white"}>10%</strong>. PBJT dikecualikan dari PPN (UU HPP) untuk mencegah pajak ganda. <strong className={isLight ? "text-blue-700 font-bold" : "text-blue-300"}>Service Charge</strong> diatur secara terpisah sebagai biaya layanan usaha restoran.
                </p>
              </div>

              {/* Status Penerapan Pajak (Capsule Switch Active - Inactive) */}
              <div className={`mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 rounded-xl border ${isLight ? "bg-slate-50 border-slate-200" : "bg-white/[0.02] border-white/10"}`}>
                <div className="space-y-1">
                  <div className={`text-xs font-bold flex items-center gap-2 ${isLight ? "text-slate-900" : "text-slate-200"}`}>
                    Status Penerapan Pajak & Tarif di POS
                    <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase ${isTaxActive ? "bg-emerald-500/20 text-emerald-500 border border-emerald-500/30" : "bg-rose-500/20 text-rose-500 border border-rose-500/30"}`}>
                      {isTaxActive ? "Active" : "Inactive"}
                    </span>
                  </div>
                  <p className={`text-[11px] ${isLight ? "text-slate-600" : "text-slate-400"}`}>
                    Jika Inactive, perhitungan pajak (PBJT/PPN) & service charge dinonaktifkan (0%) pada seluruh transaksi POS kasir dan cetak struk.
                  </p>
                </div>

                {/* Tombol Switch Kapsul Active - Inactive */}
                <div className={`flex items-center p-1 rounded-full border shrink-0 shadow-inner ${isLight ? "bg-slate-200 border-slate-300" : "bg-[#0c0c0c] border-white/10"}`}>
                  <button
                    type="button"
              ...
              <div className={`space-y-5 transition-all ${!isTaxActive ? "opacity-40 pointer-events-none" : ""}`}>
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button onClick={() => { setTaxType("pbjt"); setTaxRateInput(10); }} className={`rounded-lg border px-4 py-3 text-sm font-medium text-left transition-colors ${taxType === "pbjt" ? (isLight ? "bg-blue-50 border-blue-500 text-blue-900 shadow-xs" : "bg-blue-500/15 border-blue-500/40 text-blue-200") : (isLight ? "bg-white border-slate-200 text-slate-700 hover:bg-slate-50" : "bg-white/5 border-white/10 text-slate-400 hover:bg-white/10")}`}>
                      <div className="font-semibold">PBJT</div>
                      <div className="text-xs mt-0.5 opacity-70">Pajak Restoran (Daerah)</div>
                    </button>
                    <button onClick={() => { setTaxType("ppn"); setTaxRateInput(11); }} className={`rounded-lg border px-4 py-3 text-sm font-medium text-left transition-colors ${taxType === "ppn" ? (isLight ? "bg-blue-50 border-blue-500 text-blue-900 shadow-xs" : "bg-blue-500/15 border-blue-500/40 text-blue-200") : (isLight ? "bg-white border-slate-200 text-slate-700 hover:bg-slate-50" : "bg-white/5 border-white/10 text-slate-400 hover:bg-white/10")}`}>
                ...
                <div className="grid grid-cols-2 gap-5">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className={inputClass}
                    />
                    <p className={`text-[11px] ${isLight ? "text-slate-600" : "text-slate-500"}`}>{taxType === "pbjt" ? "Tarif PBJT daerah: 0–10%. Cek peraturan daerah Anda." : "PPN 11% sesuai UU HPP."}</p>
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className={inputClass}
                    />
                    <p className={`text-[11px] ${isLight ? "text-slate-600" : "text-slate-500"}`}>Opsional. 0–5%. Bukan pajak, adalah pendapatan restoran.</p>
                  </div>
                </div>
              </div>

              {/* Real-time tax calculation preview */}
              <div className={`mt-5 rounded-xl border p-4 transition-all ${isTaxActive ? (isLight ? "border-blue-300 bg-blue-50" : "border-blue-500/10 bg-blue-500/5") : (isLight ? "border-slate-300 bg-slate-100" : "border-slate-500/10 bg-slate-500/5")}`}>
                <p className={`text-xs font-medium mb-1 flex items-center gap-1.5 ${isTaxActive ? (isLight ? "text-blue-800" : "text-blue-300") : "text-slate-500"}`}>
                  <AlertTriangle className="size-3.5" /> Contoh kalkulasi struk ({isTaxActive ? "Pajak & Layanan Aktif" : "Pajak & Layanan Inactive"})
                </p>
                <p className={`text-xs ${isTaxActive ? (isLight ? "text-blue-900" : "text-blue-200/70") : "text-slate-500"}`}>
                  Subtotal Rp 100.000
                  {isTaxActive ? (
                    <>
                      {" → "} {TAX_LABELS[taxType]} {taxRateInput}% = Rp {(100000 * taxRateInput / 100).toLocaleString("id-ID")}
                      {serviceChargeInput > 0 && ` + Service Charge ${serviceChargeInput}% = Rp ${(100000 * serviceChargeInput / 100).toLocaleString("id-ID")}`}
                      {" → "} Total Rp {(100000 * (1 + taxRateInput / 100 + serviceChargeInput / 100)).toLocaleString("id-ID")}
                    </>
                  ) : (
                    <>
              ...
              <p className={`mt-4 text-[11px] flex items-center gap-1 ${isLight ? "text-emerald-700 font-semibold" : "text-emerald-400"}`}>
            ...
            <Glass className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    <label className={`text-xs font-medium flex items-center gap-1.5 ${isLight ? "text-emerald-800 font-bold" : "text-emerald-400"}`}>
                    ...
                    >
                      <option value="audit_full">🛡️ Wajib Otorisasi Manager & Tampilkan Item Void (Audit Full)</option>
                      <option value="zero_out">📑 Nol-kan / Sembunyikan Item Void pada Struk Pelanggan (Zero-Out)</option>
                      <option value="manager_only">🔒 Kunci Total Fitur Void Kasir (Manager Only via Dasbor)</option>
                    </select>
                  </div>
                  <p className={`text-[11px] flex items-center gap-1 ${isLight ? "text-emerald-700 font-semibold" : "text-emerald-400"}`}>
                ...
                <div className="space-y-3.5">
                  <div className={`flex items-center justify-between p-1 rounded-xl border shadow-inner gap-1 ${isLight ? "bg-slate-200 border-slate-300" : "bg-[#0c0c0c] border-white/10"}`}>
                    <button
                      type="button"
                      ...
                      className={`flex-1 py-2 px-2 rounded-lg text-[11px] font-semibold transition-all flex items-center justify-center gap-1.5 ${
                        strukPreviewMode === "transaksi"
                          ...
                          : isLight ? "text-slate-600 hover:text-slate-900 hover:bg-slate-300/50" : "text-slate-400 hover:text-slate-200"
                      }`}
                    >
                  ...
                  {strukPreviewMode === "transaksi" ? (
                    <div className="bg-[#fafafa] text-slate-950 p-5 rounded-2xl border border-slate-200 font-mono text-[11px] space-y-2 shadow-xl animate-fadeIn leading-relaxed select-none">
                      ...
                      <div className="text-center space-y-0.5">
                        ...
                        <div className="text-[10px] text-slate-700 px-2 leading-tight">{alamatInput || "Jl. Pertanian No. 57, Lebak Bulus, Jak-Sel"}</div>
                      ...
                      <div className="text-[10px] space-y-0.5">
                        ...
                        <div className="flex"><span className="w-14">Kasir</span><span>: {(employeesList || []).find(e => e.role?.toLowerCase() === "kasir" || e.role?.toLowerCase() === "manager")?.name || ownerInput || "Verena"}</span></div>
                      ...
                      {voidPolicy === "audit_full" && (
                        <div className="bg-rose-50 border border-rose-300 p-1.5 rounded text-[8px] text-rose-900 space-y-0.5 leading-tight font-sans shadow-sm animate-fadeIn">
                          ...
                          <div>Item pembatalan/void tetap dicantumkan dengan coretan merah pada struk pelanggan beserta bukti otorisasi Manager untuk transparansi total.</div>
                        </div>
                      )}
                      {voidPolicy === "zero_out" && (
                        <div className="bg-blue-50 border border-blue-300 p-1.5 rounded text-[8px] text-blue-900 space-y-0.5 leading-tight font-sans shadow-sm animate-fadeIn">
                          ...
                          <div>Item yang dibatalkan disembunyikan total (dinolkan) dari cetakan struk pelanggan agar struk tetap bersih, rapi, dan tanpa jejak pembatalan.</div>
                        </div>
                      )}
                      {voidPolicy === "manager_only" && (
                        <div className="bg-amber-50 border border-amber-300 p-1.5 rounded text-[8px] text-amber-900 space-y-0.5 leading-tight font-sans shadow-sm animate-fadeIn">
                          ...
                          <div>Tombol void dikunci total di layar Kasir POS. Pembatalan pesanan hanya dapat dieksekusi oleh Manager atau Owner melalui Dasbor Pusat.</div>
                        </div>
                      )}
                      <div className="border-b border-dashed border-slate-400 my-1" />
                      ...
                      <div className="text-center text-[10px] space-y-0.5 pt-1">
                        ...
                        <div className="font-bold leading-tight whitespace-pre-line">{strukFooter || "Dukung UMKM Indonesia\nTulang Punggung Ekonomi Nasional"}</div>
                    ...
                    <div className="bg-[#fafafa] text-slate-950 p-5 rounded-2xl border border-slate-200 font-mono text-[11px] space-y-2 shadow-xl animate-fadeIn leading-relaxed select-none">
                      ...
                      <div className="text-[11px] space-y-0.5 font-bold">
                        <div className="flex"><span className="w-16">TIPE</span><span>: DINE-IN</span></div>
                        <div className="flex"><span className="w-16">Meja</span><span>: Meja 5 (A3)</span></div>
                        <div className="flex"><span className="w-16">Jam</span><span>: {new Date().toLocaleTimeString("id-ID", { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span></div>
                        <div className="flex"><span className="w-16">Order</span><span>: #MP9S13Z4</span></div>
                      </div>
                      <div className="border-b border-dashed border-slate-400 my-1" />
                        ...
                        {voidPolicy === "audit_full" && (
                          <div className="border-t border-dashed border-slate-400 pt-2 mt-2">
                            <div className="flex items-center justify-between text-slate-900 line-through text-[10px] bg-rose-100 p-1.5 rounded border border-rose-300 font-normal">
                              ...
                              <span className="text-[8px] uppercase font-bold text-rose-900">(POS Auth: SITI MANAGER)</span>
                            </div>
                          </div>
                        )}
                        {voidPolicy === "zero_out" && (
                          <div className="border-t border-dashed border-slate-400 pt-2 mt-2">
                            <div className="flex items-center justify-between text-slate-900 line-through text-[10px] bg-blue-100 p-1.5 rounded border border-blue-300 font-normal">
                              <span className="font-bold text-blue-900">[VOID ZERO-OUT] 1x Es Jeruk Nipis</span>
                              <span className="text-[8px] uppercase font-bold text-blue-900">(Stop Masak - Disembunyikan di Tamu)</span>
                            </div>
                          </div>
                        )}
                        {voidPolicy === "manager_only" && (
                          <div className="border-t border-dashed border-slate-400 pt-2 mt-2">
                            <div className="flex items-center justify-between text-slate-900 line-through text-[10px] bg-amber-100 p-1.5 rounded border border-amber-300 font-normal">
                              ...
                              <span className="text-[8px] uppercase font-bold text-amber-900">(Otorisasi Dasbor Pusat)</span>
                            </div>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="text-center text-[9px] text-slate-700 space-y-0.5 pt-0.5">
                        <div className="font-bold">Server: {(employeesList || []).find(e => e.role?.toLowerCase() === "pelayan" || e.role?.toLowerCase() === "waiter")?.name || "Budi (Waiters)"}</div>
                        <div className="italic text-[8px] text-slate-500">Struk tercetak otomatis dari sistem POS ke Printer Dapur</div>
                      </div>
                    </div>
                  ) : (
                    <div className="bg-[#fafafa] text-slate-950 p-5 rounded-2xl border border-slate-200 font-mono text-[10px] space-y-2.5 shadow-xl leading-relaxed animate-fadeIn">
                      <div className="text-center space-y-0.5">
                        <div className="font-bold text-sm">{nameInput || tenantName || "Pawon Salam"}</div>
                        <div className="text-[9px] text-slate-600 px-2 leading-tight">{alamatInput || "Ruko Beryl Commercial, Summarecon, Jl. Bulevar Selatan No.78, Bandung"}</div>
                      </div>
                      ...
                      <div className="pt-2 space-y-1 text-center">
                        ...
                        <div className="bg-slate-200/80 p-1.5 rounded-lg border border-slate-300 text-slate-800 font-medium text-[9px] mt-1 flex justify-between items-center shadow-sm">
                          ...
                          <span className="font-bold">{new Date().toLocaleDateString("id-ID", { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' })} · {new Date().toLocaleTimeString("id-ID", { hour: '2-digit', minute: '2-digit' })} WIB</span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-1.5 pt-1 border-t border-dashed border-slate-300">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-bold uppercase bg-slate-200 px-1.5 py-0.5 rounded text-[9px]">{(employeesList || []).find(e => e.role?.toLowerCase() === "kasir" || e.role?.toLowerCase() === "manager")?.name || ownerInput || "MARIO"}</span>
                        </div>
                        <div className="flex justify-between items-start">
                          <span className="text-slate-600">Mulai Shift</span>
                          <span className="text-right font-medium">{new Date().toLocaleDateString("id-ID", { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' })}<br/><span className="text-slate-600 font-normal">Jam 09:18:00 WIB</span></span>
                        </div>
                        <div className="flex justify-between items-start">
                          <span className="text-slate-600">Selesai Shift</span>
                          <span className="text-right font-medium">{new Date().toLocaleDateString("id-ID", { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' })}<br/><span className="text-slate-600 font-normal">Jam 18:30:00 WIB</span></span>
                        </div>
                        <div className="flex justify-between font-semibold pt-1 border-t border-slate-200"><span>Jumlah Pengunjung</span><span>6 Orang</span></div>
                      </div>

                      <div className="space-y-0.5 pt-1 border-t border-dashed border-slate-300">
                        <div className="flex justify-between"><span>Terjual item</span><span>15 item</span></div>
                        <div className="flex justify-between"><span>Terjual porsi</span><span>43 porsi</span></div>
                      </div>

                      <div className="space-y-0.5 pt-1 border-t border-dashed border-slate-300">
                        <div className="flex justify-between font-semibold"><span>Total Penjualan (Gross)</span><span>Rp 835.000</span></div>
                        <div className="flex justify-between text-emerald-700 font-medium"><span>Diskon Promo / Voucher (5 trx)</span><span>-Rp 51.750</span></div>
                        <div className="flex justify-between text-rose-600 font-medium"><span>Transaksi Void (2 bill / 3 item)</span><span>-Rp 45.000</span></div>
                        <div className="flex justify-between font-bold pt-1 border-t border-slate-300 text-[11px]"><span>Penjualan Bersih (Net)</span><span>Rp 738.250</span></div>
                      </div>
                      ...
                      <div className="space-y-1.5 pt-1.5 border-t border-dashed border-slate-300">
                        <div className="font-bold text-[9px] uppercase text-slate-900 flex items-center justify-between">
                          ...
                          <span className="bg-emerald-200 text-emerald-950 px-1 py-0.5 rounded text-[8px] font-bold">100% AUTHORIZED</span>
                        </div>
                        ...
                        <div className="flex justify-between items-center text-[9px]">
                          ...
                          <span className="font-semibold text-slate-900">{voidPolicy === "zero_out" ? "Zero-Out Struk Pelanggan" : voidPolicy === "manager_only" ? "Strict Lockout POS" : "Audit Full (Tampil)"}</span>
                        </div>
                        <div className="bg-amber-100/90 border border-amber-400 p-1 rounded text-[8px] text-amber-950 mt-1 font-semibold">
                          ⚠️ [SECURITY ALERT]: 1x Void dilakukan pasca cetak Pre-Bill (Meja 5 - Rp 15.000). Telah diverifikasi Manager.
                        </div>
                      </div>

                      <div className="border-b-2 border-double border-slate-400 pt-1" />
                      <div className="font-bold text-[9px] tracking-widest text-center uppercase py-0.5">DETAIL TRANSAKSI</div>
                      ...
                      <div className="space-y-1 pt-1 text-[9px] max-h-48 overflow-y-auto pr-1">
                        <div className="flex justify-between"><span>Air Mineral</span><span>x 3</span></div>
                        <div className="flex justify-between"><span>Jeruk Peras Hangat</span><span>x 1</span></div>
                        <div className="flex justify-between"><span>Kerupuk Udang</span><span>x 1</span></div>
                        <div className="flex justify-between"><span>Mie Godog</span><span>x 4</span></div>
                        <div className="flex justify-between"><span>Mie Goreng Jawa</span><span>x 6</span></div>
                        <div className="flex justify-between"><span>Nasi Goreng Kampung</span><span>x 2</span></div>
                        <div className="flex justify-between"><span>Nasi Putih</span><span>x 2</span></div>
                        <div className="flex justify-between"><span>Nasi Soto Ayam</span><span>x 3</span></div>
                        <div className="flex justify-between"><span>Pisgor Cheese Milk Crunchy</span><span>x 2</span></div>
                        <div className="flex justify-between"><span>Pisgor Cheese Tiramisu Chru</span><span>x 1</span></div>
                        <div className="flex justify-between"><span>Rawon tanpa nasi</span><span>x 1</span></div>
                        <div className="flex justify-between"><span>Sate Telur Puyuh</span><span>x 2</span></div>
                        <div className="flex justify-between"><span>Soto Ayam</span><span>x 4</span></div>
                        <div className="flex justify-between"><span>Teh Tawar</span><span>x 5</span></div>
                        <div className="flex justify-between"><span>Tempe Goreng</span><span>x 4</span></div>
                      </div>

                      <div className="border-b-2 border-double border-slate-400 pt-1" />
                      <div className="font-bold text-[9px] tracking-widest text-center uppercase py-0.5">DETAIL PEMASUKAN</div>
                      <div className="border-b-2 border-double border-slate-400" />

                      <div className="space-y-1 pt-1">
                        <div className="flex justify-between"><span>QRIS</span><span>Rp 420.100</span></div>
                        <div className="flex justify-between"><span>TUNAI</span><span>Rp 338.150</span></div>
                        <div className="flex justify-between font-bold pt-1 border-t border-slate-300"><span>TOTAL PEMASUKAN</span><span>Rp 758.250</span></div>
                      </div>

                      <div className="border-b-2 border-double border-slate-400 pt-1" />
                      <div className="font-bold text-[9px] tracking-widest text-center uppercase py-0.5">TRANSAKSI KAS KECIL</div>
                      <div className="border-b-2 border-double border-slate-400" />

                      <div className="space-y-1 pt-1">
                        <div className="flex justify-between font-semibold"><span>KAS AWAL</span><span>Rp 560.400</span></div>
                        <div className="flex justify-between text-slate-600"><span>gula x3</span><span>(Rp 52.500)</span></div>
                        <div className="flex justify-between text-slate-600"><span>kecap x2</span><span>(Rp 40.100)</span></div>
                        <div className="flex justify-between pt-1"><span>SALDO</span><span>Rp 467.800</span></div>
                        <div className="flex justify-between font-bold text-[11px] pt-1 border-t border-slate-300"><span>TOTAL KAS</span><span>Rp 805.950</span></div>
                      </div>
          ...
          <div id="jam" className="space-y-5">
            <Glass className="p-6">
              ...
              <div className={`flex flex-wrap items-center gap-2 mb-5 pb-3 border-b ${isLight ? "border-slate-200" : "border-white/10"}`}>
                <span className={`text-[11px] font-semibold mr-1 ${isLight ? "text-slate-700" : "text-slate-400"}`}>Aksi Cepat:</span>
                <button
                  type="button"
                ...
                <button
                  type="button"
                  onClick={() => setJamOperasional(prev => prev.map(d => d.day === "Minggu" ? { ...d, isOpen: false } : { ...d, isOpen: true }))}
                ...
                <button
                  type="button"
                  onClick={() => setJamOperasional(prev => prev.map(d => ({ ...d, openTime: "09:00", closeTime: "22:00" })))}
              ...
              <div className="space-y-3">
                {jamOperasional.map((item, idx) => (
                  <div key={item.day} className={`flex items-center gap-4 p-3 rounded-xl transition-colors border ${isLight ? "bg-white border-slate-200 hover:bg-slate-50 shadow-xs" : "bg-white/[0.01] border-white/5 hover:bg-white/[0.03]"}`}>
                    <div className={`w-20 text-xs font-bold ${isLight ? "text-slate-900" : "text-slate-200"}`}>{item.day}</div>
                    <div className="flex items-center gap-2">
                      <input
                        type="time"
                        value={item.openTime}
                        onChange={e => setJamOperasional(prev => prev.map((d, i) => i === idx ? { ...d, openTime: e.target.value } : d))}
                        className={inputClass}
                      />
                      <span className={`text-xs font-medium ${isLight ? "text-slate-600" : "text-slate-500"}`}>s/d</span>
                      <input
                        type="time"
                        value={item.closeTime}
                        onChange={e => setJamOperasional(prev => prev.map((d, i) => i === idx ? { ...d, closeTime: e.target.value } : d))}
                        className={inputClass}
                      />
                    </div>
                    <div className="ml-auto flex items-center gap-2">
                      <span className={`text-[10px] font-bold uppercase ${item.isOpen ? (isLight ? "text-emerald-700" : "text-emerald-400") : "text-slate-500"}`}>
                        {item.isOpen ? "Buka" : "Tutup"}
                      </span>
                      <button
                        type="button"
                        onClick={() => setJamOperasional(prev => prev.map((d, i) => i === idx ? { ...d, isOpen: !d.isOpen } : d))}
                        className={`w-8 h-4 rounded-full border flex items-center px-0.5 transition-all cursor-pointer ${
                          item.isOpen
                            ? isLight ? "bg-emerald-200 border-emerald-500 justify-end" : "bg-emerald-500/20 border-emerald-500/40 justify-end"
                            : isLight ? "bg-slate-200 border-slate-300 justify-start" : "bg-white/5 border-white/10 justify-start"
                        }`}
                      >
                        <div className={`size-3 rounded-full transition-colors ${item.isOpen ? (isLight ? "bg-emerald-600" : "bg-emerald-400") : "bg-slate-500"}`} />
                      </button>
                    </div>
                  </div>
                ))}
              </div>
              <p className={`mt-3 text-[11px] flex items-center gap-1 ${isLight ? "text-emerald-700 font-semibold" : "text-emerald-400"}`}>✓ Jam operasional disimpan via tombol "Simpan Semua Perubahan".</p>
            </Glass>
          </div>

          {/* GLOBAL SAVE ALL CHANGES BUTTON — AT THE VERY BOTTOM FOR ALL TABS */}
          <Glass className={`p-5 sticky bottom-0 z-20 backdrop-blur-xl shadow-2xl border ${isLight ? "border-slate-300 bg-white/90" : "border-emerald-500/30 bg-gradient-to-r from-emerald-950/20 via-slate-900/40 to-slate-900/40"}`}>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div className="space-y-1">
                <div className={`font-bold text-sm flex items-center gap-2 ${isLight ? "text-slate-900" : "text-slate-100"}`}>
                  <Check className="size-4 text-emerald-500" /> Simpan Semua Pengaturan Outlet
                </div>
                <p className={descClass}>
                  Perubahan pada branding, profil, lokasi, pajak, struk, mode UI, dan jam operasional akan disimpan secara global.
                </p>
              </div>
              <div className="flex items-center gap-3 self-end sm:self-auto shrink-0">
                {saveSuccessMsg && (
                  <span className={`text-xs font-bold animate-pulse flex items-center gap-1 px-3 py-1.5 rounded-lg border ${isLight ? "text-emerald-700 bg-emerald-50 border-emerald-300" : "text-emerald-400 bg-emerald-500/10 border-emerald-500/20"}`}>
                    ✓ Berhasil Disimpan!
                  </span>
                )}
                <button 
