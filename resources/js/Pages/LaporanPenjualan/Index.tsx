...
function LaporanPenjualanInner() {
  ...
  return (
    <MainLayout>
      ...
      <Screen title="Laporan Penjualan" action={
        ...
        <div className="grid grid-cols-4 gap-5">
          {summary.map(s => (
            <Glass className="p-5" key={s.label}>
              ...
              <Badge tone={s.tone} className="mt-2">{s.delta}</Badge>
            </Glass>
          ))}
        </div>
        <Glass className="p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2 no-print">
              {["7 Hari", "30 Hari", "Bulan Ini"].map(p => (
                <button key={p} className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors border ${p === "7 Hari" ? "bg-white/10 text-white border-white/20" : "bg-white/5 text-slate-400 border-white/10 hover:bg-white/10"}`}>{p}</button>
              ))}
            </div>
          </div>
          <div className="grid grid-cols-[100px_1fr_1fr_1fr_1fr] border-b border-white/5 pb-3 text-[11px] font-medium uppercase tracking-wider text-slate-400">
            <span>Tanggal</span><span>Jumlah Order</span><span>Pendapatan</span><span>Food Cost</span><span>{TAX_LABELS.pbjt}</span>
          </div>
          {rows.map(r => (
            <div key={r.date} className="grid grid-cols-[100px_1fr_1fr_1fr_1fr] items-center border-b border-white/5 py-3.5 text-sm">
