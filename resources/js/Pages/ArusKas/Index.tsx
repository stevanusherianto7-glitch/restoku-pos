          ...
          `,
                        ...
                        <div className="hidden print:block mb-8 border-b border-white/10 pb-4">
                            ...
                            <p className="text-slate-400 text-xs mt-1">
                                Tanggal Mutasi: {selectedDate} � Cetak: {new Date().toLocaleDateString('id-ID')}
                            </p>
                        </div>

                        <h2 className="text-base font-medium text-slate-200 mb-4 print:hidden">
                            Riwayat Transaksi Kas � {selectedDate}
                        </h2>
                        <h2 className="text-base font-bold text-white mb-4 hidden print:block">
                            Riwayat Transaksi Mutasi Kas
                        </h2>
                        ...
                        {entries.map((e, i) => (
                            <div
                                key={i}
                                className="grid grid-cols-[80px_1fr_80px_1fr_1fr] items-center border-b border-white/5 py-3.5 text-sm"
                            >
                                <span className="font-mono text-slate-400 text-xs">{e.time}</span>
                                <span className="text-slate-300">{e.desc}</span>
                                <span>
                                    {e.type === 'in' ? (
                                        <Badge tone="emerald">Masuk</Badge>
                                    ) : (
                                        <Badge tone="red">Keluar</Badge>
                                    )}
                                </span>
                                <span
                                    className={`font-mono font-semibold ${e.type === 'in' ? 'text-emerald-400' : 'text-red-400'}`}
                                >
                                    {e.type === 'in' ? '+' : ''}
