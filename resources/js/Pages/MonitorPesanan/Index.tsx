...
import { 
  ShoppingBag, Printer, Clock, ArrowRight, Utensils, CheckCircle, 
  RefreshCw, Layers, CheckSquare, Sparkles, MessageCircle, CreditCard, GlassWater
} from "lucide-react";
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-6">
          <Glass className="p-5 flex items-center justify-between border-l-4 border-l-amber-500">
            <div>
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Antrean Pembayaran</p>
          ...
          <Glass className="p-5 flex items-center justify-between border-l-4 border-l-blue-500">
            <div>
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Makan Di Sini (Dine In)</p>
          ...
          <Glass className="p-5 flex items-center justify-between border-l-4 border-l-emerald-500">
            <div>
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Bawa Pulang (Take Away)</p>
              <h3 className="text-2xl font-black text-slate-100 mt-1">
                {orders.filter(o => o.table.toLowerCase().includes("takeaway")).length}
              </h3>
            </div>
        ...
        <div className="flex gap-2 mb-6">
          {(["all", "dine_in", "take_away"] as const).map(tab => (
            ...
            >
              {tab === "all" ? "Semua Pesanan" : tab === "dine_in" ? "Makan Di Sini (Dine In)" : "Bawa Pulang (Take Away)"}
            </button>
          ))}
        </div>
        ...
        {filteredOrders.length > 0 ? (
              ...
              return (
                <Glass 
                  ...
                  className={`flex flex-col justify-between overflow-hidden border transition-all ${
                    isTakeaway ? "border-emerald-500/10 hover:border-emerald-500/30" : "border-blue-500/10 hover:border-blue-500/30"
                  }`}
                >
                  <div className="p-5">
                    {/* Header */}
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <span className="text-[10px] font-mono text-slate-500 uppercase tracking-wider">{order.id}</span>
                    ...
                    <div className="space-y-2 border-y border-white/5 py-4 my-4 max-h-48 overflow-y-auto pr-1">
                      {order.items.map((item, idx) => {
                        ...
                        return (
                          <div key={idx} className={`text-xs ${isNote ? "text-amber-400 italic pl-3" : "text-slate-200"}`}>
                            {item}
                          </div>
                        );
                      })}
          ...
          <Glass className="py-20 text-center flex flex-col items-center justify-center">
            ...
            <p className="text-xs text-slate-500 mt-1 max-w-xs">
              Antrean pembayaran akan otomatis muncul di sini ketika tamu melakukan pemesanan via buku menu digital.
            </p>
          </Glass>
        )}
      </Screen>
