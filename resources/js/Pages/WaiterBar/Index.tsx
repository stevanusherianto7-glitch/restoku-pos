        ...
        {orders.length > 0 && (
          <div className="mb-6 bg-amber-500/10 border border-amber-500/20 rounded-3xl p-5 flex items-center justify-between font-mono animate-pulse">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-black text-amber-400 uppercase tracking-wider">ORDERAN MINUMAN MASUK!</h3>
                <p className="text-xs text-slate-400 mt-0.5">Segera siapkan minuman dan antarkan ke meja pelanggan.</p>
              </div>
            </div>
            ...
            {orders.map((o) => (
              <Glass className="p-6 border-amber-500/10" key={o.id}>
                <div className="mb-4 flex justify-between items-start">
                  <div>
                    <b className="text-5xl font-bold tracking-tight block text-slate-100">{o.table}</b>
                  ...
                  <span className="font-mono text-2xl opacity-90 flex items-center gap-1.5 font-bold bg-black/20 px-2.5 py-1.5 rounded-xl">
                    <Clock3 className="size-5" />{o.time}'
                  </span>
                </div>
                ...
                <div className="space-y-2 text-2xl font-medium mt-6 border-t border-white/10 pt-4">
                  {o.items.map((item, i) => {
                    ...
                    return (
                      <p key={i} className={isNote ? "text-xl opacity-70 font-normal italic pl-4 text-amber-400" : "text-slate-200"}>
                        {item}
                      </p>
                    );
                  })}
