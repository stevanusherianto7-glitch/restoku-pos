...
function InventoryInner() {
  const rows = [
    ...
    { name: "Minyak Goreng", cat: "Grocery", stock: 18, unit: "liter", cost: 17800, updated: "15 mnt lalu", low: false },
  ...
  return (
    <MainLayout>
      ...
      <Screen title="Master Bahan Baku & Stok" action={
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 flex-1 max-w-xs focus-within:border-white/20 transition-colors">
            ...
            <input placeholder="Cari bahan baku..." className="w-full bg-transparent py-2 text-sm outline-none text-slate-200 placeholder:text-slate-400" />
          </div>
          <button className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm font-medium text-slate-300 hover:bg-white/10 flex items-center gap-2">
            <SlidersHorizontal className="size-4" />Kategori
          </button>
          <button className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm font-medium text-slate-300 hover:bg-white/10">Outlet {MOCK_OUTLET}</button>
        ...
        <Glass className="overflow-hidden">
          <div className="p-5">
            <div className="grid grid-cols-[1.4fr_1fr_1.2fr_.7fr_1fr_1fr_.5fr] border-b border-white/5 pb-3 text-[11px] font-medium uppercase tracking-wider text-slate-400">
              <span>Nama Bahan</span><span>Kategori</span><span>Stok</span><span>Satuan</span><span>Harga/Satuan</span><span>Diperbarui</span><span></span>
            </div>
            {rows.map(r => (
              <div className="grid grid-cols-[1.4fr_1fr_1.2fr_.7fr_1fr_1fr_.5fr] items-center border-b border-white/5 py-4 text-sm" key={r.name}>
                ...
                <button className="text-slate-500 hover:text-slate-300 transition-colors"><Pencil className="size-3.5" /></button>
              </div>
            ))}
          </div>
        </Glass>
