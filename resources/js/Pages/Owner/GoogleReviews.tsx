...
import { Star, RefreshCw, Send, CheckCircle2, AlertTriangle, Settings, Sparkles, AlertCircle, X, Check, MessageSquare } from "lucide-react";
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base font-bold text-emerald-200">
                  Google Review & Complaint Hub
                </h3>
                <p className="text-xs text-emerald-300/70">
                  Pusat penanganan kritik, saran, dan kepuasan pelanggan secara real-time dari Google Maps.
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`text-black rounded-xl px-4 py-2.5 text-xs font-semibold flex items-center gap-2 transition-all ${
                  isNanoBanana ? "bg-amber-400 hover:bg-amber-500" : "bg-emerald-400 hover:bg-emerald-500"
                } disabled:opacity-50`}
              >
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-black/40 border border-white/5 rounded-xl">
            <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Kategori Review</label>
                <div className="flex bg-black/40 border border-white/10 rounded-lg p-0.5">
                  <button
                    onClick={() => setFilterRating("all")}
                    className={`px-3 py-1 rounded-md text-xs font-semibold transition-all ${
                      filterRating === "all" ? "bg-white/10 text-white" : "text-slate-400 hover:text-white"
                    }`}
                  >
                    Semua
                  </button>
                  <button
                    ...
                    className={`px-3 py-1 rounded-md text-xs font-semibold transition-all flex items-center gap-1.5 ${
                      filterRating === "complaints" ? "bg-red-500/20 text-red-300 border border-red-500/30" : "text-slate-400 hover:text-white"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-md text-xs font-semibold transition-all ${
                      filterRating === "positive" ? "bg-emerald-500/20 text-emerald-300 border border-emerald-500/30" : "text-slate-400 hover:text-white"
                    }`}
                  >
                    Positif (Bintang 4-5)
                  </button>
              ...
              {/* Status filter */}
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Status Balasan</label>
                <div className="flex bg-black/40 border border-white/10 rounded-lg p-0.5">
                  <button
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-md text-xs font-semibold transition-all ${
                      filterStatus === "unreplied" ? "bg-amber-500/20 text-amber-300 border border-amber-500/30" : "text-slate-400 hover:text-white"
                    }`}
                  >
                    Belum Dibalas
                  </button>
                  <button
                    ...
                    className={`px-3 py-1 rounded-md text-xs font-semibold transition-all ${
                      filterStatus === "replied" ? "bg-emerald-500/20 text-emerald-300 border border-emerald-500/30" : "text-slate-400 hover:text-white"
                    }`}
                  >
                    Sudah Dibalas
                  </button>
            ...
            <div className="text-xs text-slate-400">
              Menampilkan <span className="text-white font-bold">{filteredReviews.length}</span> dari {reviews.length} ulasan
            </div>
          </div>
          ...
          {loading ? (
            ...
            <Glass className="p-12 text-center text-slate-400 flex flex-col items-center justify-center border-white/5">
              ...
              <p className="text-xs text-slate-500 mt-1">Coba sinkronkan ulasan Anda atau sesuaikan filter pencarian.</p>
            </Glass>
          ) : (
                ...
                return (
                  <Glass
                    ...
                    className={`p-5 border-white/5 flex flex-col gap-4 ${
                      isComplaint && !review.reply_text ? "border-l-4 border-l-red-500 bg-red-500/[0.01]" : ""
                    }`}
                    hover
                  >
                      ...
                      <div className="ml-13 p-4 bg-white/[0.02] border border-white/5 rounded-xl space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-bold text-slate-400 flex items-center gap-1.5">
                            <CheckCircle2 className="size-3.5 text-emerald-400" /> Balasan dari Anda:
                          </span>
                          <span className="text-[10px] text-slate-500">
                        ...
                        {replyingToId === review.id ? (
                          <div className="space-y-3 bg-black/40 border border-white/5 rounded-xl p-4">
                            <div className="flex justify-between items-center">
                              <span className="text-xs font-bold text-white">Draft Balasan</span>
                              <button
                              ...
                              >
                                ...
                                {generatingAi ? "AI Berpikir..." : "Auto Balas dengan AI"}
                              </button>
                            </div>
                          ...
                          <div className="flex gap-2">
                            ...
                            >
                              <Sparkles className="size-3 text-amber-400 animate-pulse" /> Tulis Balasan dengan AI
                            </button>
                          </div>
                        )}
                      </div>
          ...
          {showSettings && (
            <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-zinc-950 border border-white/10 rounded-2xl w-full max-w-md p-6 relative shadow-2xl">
                ...
                <div className="space-y-4">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Google Place ID</label>
                    <input
                      type="text"
                      value={placeId}
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Google API Key</label>
                    <input
                      type="password"
                      value={apiKey}
