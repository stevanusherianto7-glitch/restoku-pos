...
function POSInner() {
  ...
  const subBillDiscount = cart.discountType === "percentage" 
    ? (subBillSubtotal * cart.discountValue) / 100 
    : cart.discountType === "nominal"
      ? cart.subtotal > 0 ? (subBillSubtotal / cart.subtotal) * cart.discountValue : 0
      : 0;
            ...
            {servedQueue.length > 0 && (
              <div className="mb-5 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[10px] text-slate-500">Klik meja untuk memproses tagihan</span>
                </div>
                    ...
                    return (
                      ...
                      >
                        <div>
                          ...
                          <div className="text-[8px] text-slate-500 mt-1 uppercase font-bold tracking-wider">Rincian Menu:</div>
                          <div className="text-[9px] text-slate-300 mt-0.5 space-y-0.5 border-t border-white/5 pt-1 max-w-[135px]">
                            {order.items.filter(i => !i.startsWith("+")).map((it: string, idx: number) => (
                              <div key={idx} className="truncate select-none font-medium leading-snug">{it}</div>
                            ))}
                          </div>
                        </div>
                        <div className="mt-3 flex flex-col gap-1.5">
                          <span className={`self-start text-[8px] px-1.5 py-0.5 rounded font-extrabold uppercase tracking-wide border ${isTakeAway ? "bg-amber-500/10 text-amber-400 border-amber-500/20" : "bg-blue-500/10 text-blue-400 border-blue-500/20"}`}>
                            {isTakeAway ? "Take Away" : "Dine In"}
                          </span>
                          <div className="text-xs font-bold text-emerald-400 font-mono">{formatRupiah(calculateOrderTotal(order))}</div>
                        </div>
                      </button>
                    );
                  })}
            ...
            <div className="grid grid-cols-4 gap-5 flex-1 overflow-y-auto pr-2 pb-4">
              {filteredMenu.map(product => {
                ...
                return (
                  ...
                  >
                    {inCart && (
                      <div className={`absolute top-2 right-2 z-10 size-5 rounded-full text-[10px] font-bold flex items-center justify-center shadow-lg ${
                        isNanoBanana ? "bg-amber-500 text-slate-950 ring-2 ring-[#030712]" : "bg-blue-500 text-white"
                      }`}>
                        {inCart.quantity}
                      </div>
                    )}
                    <div className="h-28 w-full relative overflow-hidden bg-slate-900 border-b border-white/5 flex items-center justify-center">
                      <ProductImage src={"image" in product ? product.image || null : null} alt={product.name} variant="full" className="w-full h-full object-cover rounded-none transition-transform duration-300 group-hover:scale-105" />
                    </div>
                    <div className="p-3">
                      <p className="font-semibold text-sm text-slate-200 leading-tight truncate">{product.name}</p>
                      <p className={`font-mono text-xs mt-1.5 font-bold ${isNanoBanana ? "text-amber-400" : "text-emerald-400"}`}>{formatRupiah(product.price)}</p>
                    </div>
                  </button>
                );
              })}
          ...
          <Glass className="p-5 flex flex-col h-[calc(100vh-160px)]">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2 shrink-0">
                {activeOrderId && (
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase ${orderTableName?.toLowerCase().includes("takeaway") ? "bg-amber-500/10 text-amber-400 border border-amber-500/20" : "bg-blue-500/10 text-blue-400 border border-blue-500/20"}`}>
                    {orderTableName?.toLowerCase().includes("takeaway") ? "🛍️ Take Away" : "🍽️ Dine In"}
                  </span>
                )}
                {cart.itemCount > 0 && (
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-1 min-h-0">
              ...
              {cart.items.map(item => (
                <div key={item.id} className={`rounded-2xl border transition-all duration-300 ease-out p-3.5 shadow-sm ${
                  isNanoBanana
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1 min-w-0">
                      ...
                      {item.note && (
                        <p className="text-[11px] text-amber-400/80 mt-0.5 truncate">📝 {item.note}</p>
                      )}
                      <p className="text-slate-500 text-xs mt-0.5">{formatRupiah(item.price)} / porsi</p>
                    </div>
                    <div className="flex items-center gap-1.5 ml-3 shrink-0">
                      ...
                      <span className="font-mono text-sm w-5 text-center text-slate-200">{item.quantity}</span>
                      <button type="button"
                        onClick={() => cart.addItem({ id: item.id, name: item.name, price: item.price, category: item.category })}
                    ...
                    {editingNoteId === item.id ? (
                      <div className="flex gap-1.5 flex-1 mr-2">
                        <input
                          ...
                          onKeyDown={e => { if (e.key === "Enter") handleNoteSubmit(item.id); if (e.key === "Escape") setEditingNoteId(null); }}
                        ...
                        />
                        <button type="button" onClick={() => handleNoteSubmit(item.id)}
            ...
            <div className="pt-4 border-t border-white/10 mt-4 space-y-3">
              ...
              {cart.discountAmount > 0 && (
                <div className="flex justify-between text-sm text-emerald-400">
                  <span className="flex items-center gap-1 cursor-pointer hover:text-emerald-300" onClick={() => { setTempDiscountType(cart.discountType); setTempDiscountValue(cart.discountValue.toString()); setIsDiscountModalOpen(true); }}>
                    Diskon {cart.discountType === "percentage" ? `(${cart.discountValue}%)` : ""} <Pencil className="size-3" />
                  </span>
              ...
              {cart.discountAmount === 0 && (
                <div className="flex justify-between text-sm text-slate-400">
                  <button type="button" onClick={() => setIsDiscountModalOpen(true)} className="text-blue-400 hover:text-blue-300 flex items-center gap-1 transition-colors">
                    <Percent className="size-3" /> Tambah Diskon
                  </button>
                </div>
              )}
              <div className="flex justify-between text-sm text-slate-400">
                <span>{TAX_LABELS[taxType as keyof typeof TAX_LABELS] ?? "Pajak"} {taxRate}%</span>
              ...
              <div className="space-y-2 pt-1">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {paymentMethods.map(([key, label]) => (
                    <button key={key} type="button" onClick={() => setSelectedPayment(key)}
              ...
              <div className="flex gap-2 mt-2">
                ...
                >
                  {isSendingWa && <span className="animate-spin size-4 border-2 border-current border-t-transparent rounded-full" />}
                  {waSent ? "Pesanan & WA Terkirim!" : (cart.itemCount > 0 ? `Bayar · ${formatRupiah(total)}` : "Pembayaran")}
                </button>
              </div>
            </div>
          </Glass>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <Glass className="p-5 max-w-sm w-full">
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setIsDiscountModalOpen(false)} className="text-slate-400 hover:text-white"><XCircle className="size-4" /></button>
              </div>
              <div className="flex gap-2 mb-4">
                <button onClick={() => setTempDiscountType("percentage")} className={`flex-1 py-2 text-xs font-medium rounded-lg border ${tempDiscountType === "percentage" ? "bg-blue-500/20 text-blue-300 border-blue-500/40" : "bg-white/5 text-slate-400 border-white/10"}`}>Persentase (%)</button>
                <button onClick={() => setTempDiscountType("nominal")} className={`flex-1 py-2 text-xs font-medium rounded-lg border ${tempDiscountType === "nominal" ? "bg-blue-500/20 text-blue-300 border-blue-500/40" : "bg-white/5 text-slate-400 border-white/10"}`}>Nominal (Rp)</button>
              </div>
              ...
              <div className="flex gap-2">
                <button onClick={() => { cart.setDiscount("none", 0); setIsDiscountModalOpen(false); }} className="flex-1 rounded-lg py-2 text-xs font-medium bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-colors">Hapus Diskon</button>
                <button onClick={() => { cart.setDiscount(tempDiscountType, Number(tempDiscountValue)); setIsDiscountModalOpen(false); }} className="flex-1 rounded-lg py-2 text-xs font-medium bg-blue-600 hover:bg-blue-500 text-white transition-colors">Terapkan</button>
              </div>
            </Glass>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <Glass className="p-6 max-w-md w-full max-h-[90vh] flex flex-col">
              <div className="flex justify-between items-center mb-4 shrink-0">
                ...
                <button onClick={() => setIsSplitBillModalOpen(false)} className="text-slate-400 hover:text-white"><XCircle className="size-5" /></button>
              </div>

              <div className="flex gap-2 mb-4 shrink-0">
                <button onClick={() => setSplitMode("evenly")} className={`flex-1 py-2 text-xs font-medium rounded-lg border transition-colors ${splitMode === "evenly" ? "bg-blue-500/20 text-blue-300 border-blue-500/40" : "bg-white/5 text-slate-400 border-white/10"}`}>Bagi Rata</button>
                <button onClick={() => setSplitMode("per_item")} className={`flex-1 py-2 text-xs font-medium rounded-lg border transition-colors ${splitMode === "per_item" ? "bg-blue-500/20 text-blue-300 border-blue-500/40" : "bg-white/5 text-slate-400 border-white/10"}`}>Pilih Item</button>
              </div>
              ...
              {splitMode === "evenly" ? (
                <div className="mb-4 space-y-4">
                  <div className="flex justify-between items-center bg-white/5 rounded-lg p-3 border border-white/10">
                    ...
                    <span className="font-mono text-lg font-semibold text-white">{formatRupiah(total)}</span>
                  </div>
                  <div>
                    <label className="text-xs font-medium text-slate-400 block mb-2">Dibagi Berapa Orang?</label>
                    <div className="flex items-center gap-3">
                      <button onClick={() => setSplitCount(prev => Math.max(2, parseInt(prev) - 1).toString())} className="size-10 rounded-lg bg-white/5 text-slate-300 hover:bg-white/10 flex items-center justify-center border border-white/10">-</button>
                      <input type="number" value={splitCount} onChange={e => setSplitCount(e.target.value)} className="w-16 h-10 text-center bg-transparent border border-white/20 rounded-lg text-white font-mono text-lg outline-none focus:border-blue-500" min="2" max="20" />
                      <button onClick={() => setSplitCount(prev => (parseInt(prev) + 1).toString())} className="size-10 rounded-lg bg-white/5 text-slate-300 hover:bg-white/10 flex items-center justify-center border border-white/10">+</button>
                    </div>
                  </div>
                  <div className="flex justify-between items-center bg-blue-500/10 rounded-lg p-3 border border-blue-500/20">
                    ...
                    <span className="font-mono text-xl font-bold text-blue-400">{formatRupiah(Math.ceil(total / parseInt(splitCount || "1")))}</span>
                  </div>
                </div>
              ) : (
                <div className="flex flex-col flex-1 min-h-0">
                  <div className="flex-1 overflow-y-auto space-y-2 pr-1 min-h-0 mb-4">
                    {cart.items.map(item => {
                      ...
                      return (
                        <div key={item.id} className="flex justify-between items-center bg-white/5 rounded-lg p-3 border border-white/10">
                          <div className="min-w-0 flex-1">
                            <p className="text-sm font-medium text-slate-200 truncate">{item.name}</p>
                            <p className="text-xs text-slate-400 mt-0.5">{formatRupiah(item.price)}</p>
                          </div>
                          <div className="flex items-center gap-2 ml-3">
                            <button type="button" onClick={() => setSplitSelectedItems(prev => ({...prev, [item.id]: Math.max(0, selectedQty - 1)}))} className="size-7 rounded bg-white/5 text-slate-300 hover:bg-white/10 flex items-center justify-center border border-white/10">-</button>
                            <span className="font-mono text-sm w-4 text-center text-slate-200">{selectedQty}</span>
                            <button type="button" onClick={() => setSplitSelectedItems(prev => ({...prev, [item.id]: Math.min(item.quantity, selectedQty + 1)}))} className="size-7 rounded bg-white/10 text-slate-200 hover:bg-white/20 flex items-center justify-center border border-white/10">+</button>
                          </div>
                        </div>
                      )
                    })}
                  ...
                  <div className="bg-blue-500/10 rounded-lg p-3 border border-blue-500/20 shrink-0 space-y-2">
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-blue-500/20">
                      <span className="text-sm font-medium text-blue-200">Total Tagihan Parsial</span>
                      <span className="font-mono text-xl font-bold text-blue-400">{formatRupiah(subBillTotal)}</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="flex gap-2 mt-2 shrink-0">
                <button onClick={() => setIsSplitBillModalOpen(false)} className="flex-1 rounded-lg py-3 text-sm font-medium bg-white/5 hover:bg-white/10 text-slate-300 transition-colors">Tutup</button>
                {splitMode === "per_item" && subBillTotal > 0 && (
                  <button onClick={handlePayPartial} className="flex-1 rounded-lg py-3 text-sm font-medium bg-emerald-600 hover:bg-emerald-500 text-white transition-colors">Bayar & Kurangi</button>
                )}
              </div>
            </Glass>
          </div>
...
export default function POS() {
  return (
    <RoleGuard allowedRoles={["kasir","manager","owner"]} pageName="POS Kasir" allowedRoleLabel="Kasir, Manager, Owner">
      <POSInner />
    </RoleGuard>
  );
}