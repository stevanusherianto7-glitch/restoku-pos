...
import {
  CalendarDays, Clock3, Users, Phone, CheckCircle2,
  XCircle, Calendar, MapPin, PartyPopper, RefreshCw,
  ChevronDown, Filter, Search, Cake, Utensils, Star
} from "lucide-react";
...
const MOCK_RESERVATIONS: Reservation[] = [
  { id: "RSV-001", name: "Budi Santoso",    phone: "0812-3456-7890", date: "2026-07-10", time: "18:30", guests: 6,  type: "ulang_tahun",  notes: "Mohon siapkan kue ulang tahun & dekorasi balon", status: "confirmed",  created_at: Date.now() - 7200000 },
  { id: "RSV-002", name: "Dewi Rahayu",     phone: "0857-8765-4321", date: "2026-07-08", time: "12:00", guests: 2,  type: "meja",         notes: "Anniversary, preferably window seat",           status: "pending",    created_at: Date.now() - 3600000 },
  { id: "RSV-003", name: "PT Maju Bersama", phone: "0811-2233-4455", date: "2026-07-15", time: "10:00", guests: 25, type: "acara_kantor",  notes: "Team lunch bulanan, butuh ruang semi-privat",   status: "pending",    created_at: Date.now() - 1800000 },
  { id: "RSV-004", name: "Rizky Pratama",   phone: "0878-9900-1122", date: "2026-07-12", time: "19:00", guests: 12, type: "gathering",    notes: "Reuni alumni SMA, mohon siapkan area khusus",  status: "confirmed",  created_at: Date.now() - 86400000 },
  { id: "RSV-005", name: "Anita & Brama",   phone: "0821-5544-6677", date: "2026-07-20", time: "17:30", guests: 50, type: "pernikahan",   notes: "Bridal shower + makan malam, tema elegant",    status: "pending",    created_at: Date.now() - 43200000 },
  { id: "RSV-006", name: "Siti Nurhaliza",  phone: "0831-7788-9900", date: "2026-07-07", time: "13:00", guests: 4,  type: "meja",         notes: "",                                             status: "completed",  created_at: Date.now() - 172800000 },
];
...
function MonitorReservasiContent() {
  ...
  return (
    <MainLayout>
      ...
      <Screen
        title="Monitor Reservasi"
        subtitle="Kelola pemesanan meja dan acara dari tamu"
      >
        {/* Stats Row */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {[
            { label: "Menunggu Konfirmasi", value: stats.pending,   icon: Clock3,       cls: "text-amber-400",   bg: "bg-amber-500/10 border-amber-500/20"   },
            { label: "Dikonfirmasi",         value: stats.confirmed, icon: CheckCircle2, cls: "text-emerald-400", bg: "bg-emerald-500/10 border-emerald-500/20" },
            { label: "Total Reservasi",      value: stats.total,     icon: CalendarDays, cls: "text-blue-400",    bg: "bg-blue-500/10 border-blue-500/20"     },
            { label: "Total Tamu",           value: `${stats.guests} org`, icon: Users, cls: "text-purple-400",  bg: "bg-purple-500/10 border-purple-500/20"  },
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
            ...
            {todayReservations.length > 0 && (
              <Section title="Hari Ini" badge={todayReservations.length} badgeCls="bg-emerald-500/20 text-emerald-400">
                {todayReservations.map(r => <ReservationCard key={r.id} r={r} onUpdate={updateStatus} />)}
              </Section>
            )}
            {/* Upcoming */}
            {upcomingReservations.length > 0 && (
              <Section title="Mendatang" badge={upcomingReservations.length} badgeCls="bg-blue-500/20 text-blue-400">
                {upcomingReservations.map(r => <ReservationCard key={r.id} r={r} onUpdate={updateStatus} />)}
              </Section>
            )}
            {/* Past */}
            {pastReservations.length > 0 && (
              <Section title="Selesai / Lampau" badge={pastReservations.length} badgeCls="bg-slate-500/20 text-slate-400">
                {pastReservations.map(r => <ReservationCard key={r.id} r={r} onUpdate={updateStatus} />)}
              </Section>
            )}
          </div>
        )}
...
// ─── Sub-components ────────────────────────────────────────────────────────────
function Section({ title, badge, badgeCls, children }: { title: string; badge: number; badgeCls: string; children: React.ReactNode }) {
  return (
    <div>
...
function ReservationCard({ r, onUpdate }: { r: Reservation; onUpdate: (id: string, s: ReservationStatus) => void }) {
  ...
  const dateLabel = new Date(r.date).toLocaleDateString("id-ID", { weekday: "long", day: "numeric", month: "long", year: "numeric" });
  ...
  return (
    <Glass className="p-4 hover:bg-white/[0.04] transition-all duration-200">
        ...
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              <span className="text-[10px] text-slate-500 bg-white/5 border border-white/10 px-2 py-0.5 rounded-full">{TYPE_LABEL[r.type]}</span>
            </div>
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">
              <span className="flex items-center gap-1"><CalendarDays className="size-3" /> {dateLabel}</span>
              <span className="flex items-center gap-1"><Clock3 className="size-3" /> {r.time} WIB</span>
              <span className="flex items-center gap-1"><Users className="size-3" /> {r.guests} tamu</span>
      ...
      <div className="mt-2 pt-2 border-t border-white/5 flex justify-between text-[10px] text-slate-600">
        <span>ID: {r.id}</span>
        <span>Diterima: {new Date(r.created_at).toLocaleString("id-ID", { dateStyle: "short", timeStyle: "short" })}</span>
      </div>
    </Glass>
  );
}
