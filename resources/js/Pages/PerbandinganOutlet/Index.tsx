...
function PerbandinganOutletInner() {
    ...
    const outlets = [
        { name: "Cabang A", revenue: 18200000, orders: 520, aov: 35000, growth: 8, color: "text-blue-400", bg: "bg-blue-500", border: "border-blue-500/30", icon: "bg-blue-500/20" },
        { name: "Cabang B", revenue: 15500000, orders: 410, aov: 37800, growth: 15, color: "text-emerald-400", bg: "bg-emerald-500", border: "border-emerald-500/30", icon: "bg-emerald-500/20" },
        { name: "Cabang C", revenue: 12100000, orders: 320, aov: 37800, growth: -3, color: "text-amber-400", bg: "bg-amber-500", border: "border-amber-500/30", icon: "bg-amber-500/20" },
    ];
    ...
    return (
        <MainLayout>
            ...
            <div className="flex justify-between items-center mb-8 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
                ...
                <div className="flex gap-3">
                    <select className="bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm font-medium text-slate-200 focus:border-blue-500/50 outline-none backdrop-blur-md transition-colors hover:bg-white/10">
                        <option value="all" className="bg-slate-900">Outlet: Semua</option>
                        <option value="1" className="bg-slate-900">Cabang A</option>
                        <option value="2" className="bg-slate-900">Cabang B</option>
                        <option value="3" className="bg-slate-900">Cabang C</option>
                    </select>
                    <select className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl px-4 py-2.5 text-sm font-medium text-emerald-400 focus:border-emerald-500/50 outline-none transition-colors hover:bg-emerald-500/20">
                        <option value="jul2026" className="bg-slate-900">Juli 2026</option>
                        <option value="jun2026" className="bg-slate-900">Juni 2026</option>
                    </select>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                {outlets.map((o) => (
                    <Glass key={o.name} className={`p-6 border-t-2 ${o.border} hover:-translate-y-1 transition-transform duration-300`}>
                        <div className="flex justify-between items-start mb-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div className={`text-sm font-medium flex items-center gap-1 mt-0.5 ${o.growth >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                        {o.growth >= 0 ? <TrendingUp className="size-3" /> : <TrendingDown className="size-3" />}
                                        {o.growth > 0 ? '+' : ''}{o.growth}% vs bulan lalu
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div key={o.name}>
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <div className="text-right">
                                        <span className="text-base font-bold text-white block">{formatRupiah(o.revenue)}</span>
                                        <span className={`text-xs font-semibold ${o.color}`}>{pct}% dari total</span>
                                    </div>
                                </div>
                                <div className="h-3 rounded-full bg-white/5 overflow-hidden">
                                    <div className={`h-full rounded-full ${o.bg} relative overflow-hidden`} style={{ width: `${pct}%` }}>
            ...
            <Glass className="p-6">
                <div className="flex items-center gap-4 bg-blue-500/10 border border-blue-500/20 p-4 rounded-xl text-blue-200">
                    ...
                    <p className="text-sm">
                        ...
                        Cabang B memiliki pertumbuhan tertinggi (+15%) bulan ini. Pertimbangkan untuk mereplikasi strategi promosi Cabang B ke Cabang C yang sedang mengalami penurunan (-3%).
                    </p>
                </div>
            </Glass>
