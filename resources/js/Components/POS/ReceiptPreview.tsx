        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-950 flex justify-center">
          <div className="w-full max-w-[280px] bg-white text-black p-4 font-mono text-[10px] sm:text-[11px] leading-relaxed shadow-md border-t-4 border-t-slate-300 border-b-4 border-b-slate-300 filter drop-shadow-[0_2px_4px_rgba(255,255,255,0.1)] select-none">
            ...
            <div className="text-[10px] space-y-0.5">
              <div className="flex"><span className="w-14">Tgl</span><span>: {orderData.date?.split(" ")[0] || new Date().toLocaleDateString("id-ID")}</span></div>
              <div className="flex"><span className="w-14">Jam</span><span>: {orderData.date?.split(" ")[1] || new Date().toLocaleTimeString("id-ID", { hour: '2-digit', minute: '2-digit' })} WIB</span></div>
              <div className="flex"><span className="w-14">No</span><span>: #{orderData.orderId}</span></div>
              <div className="flex"><span className="w-14">Kasir</span><span>: {orderData.cashierName || "Verena"}</span></div>
              <div className="flex"><span className="w-14">Meja</span><span>: {orderData.table || "Meja 1"} (DINE-IN)</span></div>
            </div>
              ...
              {orderData.items.map(item => (
                <div key={item.id}>
                  <div className="flex justify-between font-bold">
                    ...
                    <span className="w-16 text-right">{formatRupiah(item.price).replace("Rp", "").trim()}</span>
                    <span className="w-16 text-right">{formatRupiah(item.price * item.qty).replace("Rp", "").trim()}</span>
                  </div>
                  {item.note && (
                    <div className="text-[9px] text-slate-700 font-normal italic pl-2">({item.note})</div>
                  )}
                </div>
              ))}
            </div>
