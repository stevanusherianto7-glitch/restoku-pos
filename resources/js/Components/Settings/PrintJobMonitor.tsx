    ...
    return (
        ...
        >
            <div className="grid grid-cols-4 gap-5 mb-5">
                <Glass
                    className={`p-5 flex items-center justify-between border-l-4 border-l-blue-500 ${isLight ? 'bg-white shadow-sm border-y border-r border-slate-200' : ''}`}
                >
                    <div>
                        <p className={`text-sm font-medium ${isLight ? 'text-slate-600' : 'text-slate-400'}`}>
                            Total Print Hari Ini
                        </p>
                        <h3 className={`text-2xl font-bold mt-1 ${isLight ? 'text-slate-900' : 'text-slate-100'}`}>
                            142
                        </h3>
                    </div>
                ...
                >
                    <div>
                        ...
                        <h3 className={`text-2xl font-bold mt-1 ${isLight ? 'text-emerald-700' : 'text-emerald-400'}`}>
                            138
                        </h3>
                    </div>
                ...
                >
                    <div>
                        ...
                        <h3 className={`text-2xl font-bold mt-1 ${isLight ? 'text-amber-700' : 'text-amber-400'}`}>
                            1
                        </h3>
                    </div>
            ...
            <Glass className="overflow-hidden">
                <div
                    className={`p-4 border-b flex items-center justify-between ${isLight ? 'border-slate-200 bg-slate-50' : 'border-white/5 bg-white/[0.01]'}`}
                >
                ...
                <div className="p-5">
                    <div
                        className={`grid grid-cols-[1fr_1.5fr_1fr_1.5fr_1fr_1.5fr_1.2fr] border-b pb-3 text-[11px] font-medium uppercase tracking-wider ${isLight ? 'border-slate-200 text-slate-600 font-bold' : 'border-white/5 text-slate-400'}`}
                    >
                        <span>Job ID</span>
                    ...
                    {jobs.map((job) => (
                        <div
                            className={`grid grid-cols-[1fr_1.5fr_1fr_1.5fr_1fr_1.5fr_1.2fr] items-center border-b py-3.5 text-sm ${isLight ? 'border-slate-100 text-slate-800 font-medium' : 'border-white/5 text-slate-300'}`}
                            key={job.id}
                        >
                            <span className={`font-mono ${isLight ? 'text-slate-900 font-bold' : 'text-slate-300'}`}>
                                {job.id}
                            </span>
                            <span
                            ...
                            <div className="flex flex-col pr-3">
                                <span
                                    className={`text-xs ${isLight ? 'text-slate-500 font-medium' : 'text-slate-400'}`}
                                >
                                    {job.time}
                                </span>
                                {job.error && (
