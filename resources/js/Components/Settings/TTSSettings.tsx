...
export function TTSSettings() {
  ...
  return (
    <Screen title="Pengaturan Text-to-Speech (TTS)" action={
      ...
      <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {/* Main Toggle */}
        <Glass className={`p-6 border ${isLight ? "border-emerald-300 bg-emerald-50 shadow-sm" : "border-emerald-500/30 bg-emerald-950/20"}`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className={`p-3 rounded-xl ${config.enabled ? (isLight ? 'bg-emerald-600 text-white shadow-md' : 'bg-emerald-500 text-white') : (isLight ? 'bg-slate-200 text-slate-500' : 'bg-slate-800 text-slate-400')}`}>
                <Volume2 className="size-6" />
              </div>
              <div>
                <h2 className={`text-xl font-bold mb-1 ${isLight ? "text-slate-900" : "text-white"}`}>Aktifkan Pengumuman Suara</h2>
                <p className={`text-sm ${isLight ? "text-slate-600" : "text-slate-400"}`}>Sistem akan membacakan notifikasi pesanan dan sapaan melalui speaker kasir/dapur.</p>
              </div>
            </div>
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 transition-opacity duration-300 ${!config.enabled && 'opacity-50 pointer-events-none'}`}>
          
          {/* Audio Controls */}
          <Glass className="p-6 space-y-6">
            <h3 className={`text-lg font-bold flex items-center gap-2 mb-4 ${isLight ? "text-slate-900" : "text-white"}`}>
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-sm mb-2">
                  <span className={isLight ? "text-slate-700 font-bold" : "text-slate-300"}>Volume Pengumuman</span>
                  <span className={`font-bold ${isLight ? "text-blue-600" : "text-blue-400"}`}>{Math.round(config.volume * 100)}%</span>
                </div>
                <input 
                  type="range" 
              ...
              <div>
                <div className="flex justify-between text-sm mb-2">
                  <span className={isLight ? "text-slate-700 font-bold" : "text-slate-300"}>Kecepatan Bicara</span>
                  <span className={`font-bold ${isLight ? "text-blue-600" : "text-blue-400"}`}>{config.rate}x</span>
                </div>
                <input 
                  type="range" 
                  ...
                  className={`w-full accent-blue-500 h-2 rounded-lg appearance-none cursor-pointer ${isLight ? "bg-slate-200" : "bg-slate-800"}`}
                />
                <div className={`flex justify-between text-xs mt-1 ${isLight ? "text-slate-500 font-medium" : "text-slate-500"}`}>
                  <span>Lambat</span>
                  <span>Normal</span>
                  <span>Cepat</span>
                </div>
            ...
            <div className={`pt-4 border-t space-y-3 ${isLight ? "border-slate-200" : "border-white/10"}`}>
              <h4 className={`text-sm font-medium mb-2 ${isLight ? "text-slate-800 font-bold" : "text-slate-300"}`}>Engine TTS</h4>
              ...
              {config.engine === 'google' && (
                <div className={`p-3 rounded-lg text-xs mt-3 border ${isLight ? "bg-amber-50 border-amber-300 text-amber-900 font-semibold" : "bg-amber-500/10 border-amber-500/20 text-amber-300"}`}>
          ...
          <Glass className="p-6">
            ...
            <div className="space-y-4">
              {[
                { key: 'orderReady', label: 'Pesanan Siap Diambil', desc: 'Saat staff klik "Siap Diambil" di KDS.' },
                { key: 'queueCall', label: 'Panggilan Antrean', desc: 'Untuk order Takeaway.' },
                { key: 'welcomeScan', label: 'Sambutan Scan QR', desc: 'Diucapkan di HP tamu (opsional).' },
                { key: 'promo', label: 'Pengumuman Promo', desc: 'Otomatis saat jam sepi.' },
              ].map(event => (
                <div key={event.key} className={`p-4 rounded-xl border flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${isLight ? "bg-white border-slate-200 shadow-xs" : "bg-white/5 border-white/10"}`}>
                  <div className="flex items-start gap-3">
                    ...
                    />
                    <div>
                      <div className={`text-sm font-medium ${isLight ? "text-slate-900 font-bold" : "text-white"}`}>{event.label}</div>
                      <div className={`text-xs ${isLight ? "text-slate-600" : "text-slate-400"}`}>{event.desc}</div>
                    </div>
                  </div>
            ...
            <div className={`mt-6 p-4 rounded-xl border ${isLight ? "bg-slate-100 border-slate-300" : "bg-slate-900 border-slate-700"}`}>
              <h4 className={`text-xs font-semibold uppercase tracking-wider mb-2 ${isLight ? "text-slate-700" : "text-slate-400"}`}>Browser Support Status</h4>
              <div className="flex items-center gap-2 text-sm">
                {'speechSynthesis' in window ? (
                  <><CheckCircle2 className="size-4 text-emerald-500" /> <span className={isLight ? "text-emerald-800 font-bold" : "text-emerald-400"}>Web Speech API Tersedia</span></>
                ) : (
                  <span className="text-red-500 font-bold">Browser tidak mendukung Web Speech API</span>
                )}
              </div>
