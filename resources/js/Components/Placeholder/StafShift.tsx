...
const loadEmployeesFromStorage = (): Employee[] => {
      ...
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          return parsed.map((e: any) => ({
            ...
            role: e.role === "kasir" ? "Kasir" : (e.role === "kitchen" ? "Chef" : (e.role === "waiter" ? "Waiter" : "Chef")),
  ...
  return [
    { id: "EMP-01", name: getStaffName("kasir", "BUDI HARTONO"), role: "Kasir", avatar: getInitials(getStaffName("kasir", "BUDI HARTONO")) },
    { id: "EMP-02", name: getStaffName("waiter", "SARI PERTIWI"), role: "Waiter", avatar: getInitials(getStaffName("waiter", "SARI PERTIWI")) },
    { id: "EMP-03", name: getStaffName("kitchen", "DEDI CAHYONO"), role: "Chef", avatar: getInitials(getStaffName("kitchen", "DEDI CAHYONO")) }
  ];
};
      ...
      <div className="bg-[#1c1917]/80 rounded-2xl border border-white/5 p-6 print:bg-white print:text-black print:border-none print:shadow-none">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <Link href="/dashboard" className="p-2 rounded-lg bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white transition-colors print:hidden">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 print:text-slate-600">RENCANA & MANAJEMEN OPERASIONAL</p>
            </div>
          </div>
        ...
        <div className="flex gap-4 mt-6 pt-4 border-t border-white/5 text-[11px] font-semibold text-slate-400 print:border-slate-300 print:text-slate-800">
          <span className="flex items-center gap-1.5">
            <span className="size-3 rounded-full flex items-center justify-center text-[9px] font-bold text-white bg-blue-600">P</span> Pagi
          </span>
          <span className="flex items-center gap-1.5">
            <span className="size-3 rounded-full flex items-center justify-center text-[9px] font-bold text-white bg-emerald-600">S</span> Siang
          </span>
          <span className="flex items-center gap-1.5">
            <span className="size-3 rounded-full flex items-center justify-center text-[9px] font-bold text-white bg-cyan-600">M</span> Malam
          </span>
          <span className="flex items-center gap-1.5">
            <span className="size-3 rounded-full flex items-center justify-center text-[9px] font-bold text-white bg-rose-600">O</span> Off
          </span>
        </div>
      </div>
      ...
      {isReadOnly && (
        <div className="flex items-start gap-3 bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 text-amber-400 text-xs leading-relaxed">
          ...
          <p>
            ...
            <strong>Manager</strong> atau <strong>Owner</strong> yang berwenang menyunting atau menyimpan jadwal shift kerja.
          </p>
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          <Glass className="p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {EMPLOYEES.map(emp => (
                <div key={emp.id} className="flex items-center justify-between p-4 bg-white/[0.02] border border-white/5 rounded-xl">
                  <div className="flex items-center gap-3">
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-white/5 bg-black/20">
                    <th className="p-4 text-left text-xs font-semibold text-slate-400 w-[180px] sticky left-0 bg-[#0c0a09] z-10">KARYAWAN</th>
                    {MONTH_DAYS.map(day => (
                      <th key={day.key} className={`p-2 text-center text-[10px] font-semibold min-w-[36px] ${day.isWeekend ? 'bg-rose-500/5 text-rose-400' : 'text-slate-400'}`}>
                  ...
                  {EMPLOYEES.map(emp => (
                    <tr key={emp.id} className="border-b border-white/5 hover:bg-white/[0.01]">
                      ...
                      {MONTH_DAYS.map(day => {
                        ...
                        return (
                          <td key={day.key} className={`p-1.5 text-center ${day.isWeekend ? 'bg-rose-500/5' : ''}`}>
                            <button 
          ...
          <Glass className="p-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              ...
              <div className="flex gap-3 text-[10px] font-semibold text-slate-400">
                <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-emerald-500"></span> HADIR</span>
                <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-blue-500"></span> SAKIT</span>
                <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-amber-500"></span> IZIN</span>
                <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-rose-500"></span> ALPHA</span>
              </div>
            </div>
            ...
            <div className="overflow-x-auto custom-scrollbar">
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-white/5 bg-black/20">
                    <th className="p-3 text-left text-xs font-semibold text-slate-400 w-[180px] sticky left-0 bg-[#0c0a09] z-10">KARYAWAN</th>
                    {Array.from({ length: 31 }, (_, i) => i + 1).map(day => (
                      <th key={day} className="p-2 text-center text-[10px] font-semibold min-w-[32px] text-slate-400">
                  ...
                  {EMPLOYEES.map(emp => (
                    <tr key={emp.id} className="border-b border-white/5 hover:bg-white/[0.01]">
                      ...
                      {Array.from({ length: 31 }, (_, i) => i + 1).map(day => {
                        ...
                        if (status === "S") statusColor = "bg-blue-500/10 text-blue-400 border-blue-500/20";
                        else if (status === "I") statusColor = "bg-amber-500/10 text-amber-400 border-amber-500/20";
                        else if (status === "A") statusColor = "bg-rose-500/10 text-rose-400 border-rose-500/20";
      ...
      {viewType === "weekly" && (
        <div className="space-y-6">
          ...
          <div className="flex gap-3 bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 text-amber-400 text-xs leading-relaxed">
            ...
            <p>
              <strong>ATUR JADWAL STANDAR UNTUK 1 MINGGU.</strong> POLA INI AKAN DIULANG SETIAP MINGGU. DALAM SEBULAN SETIAP SENIN AKAN MENDAPAT POLA SENIN, DST. UNTUK MEMPERCEPAT PENGISIAN JADWAL.
            </p>
          </div>
          ...
          <Glass className="p-0 overflow-hidden border border-white/5">
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-white/5 bg-black/20">
                  <th className="p-4 text-left text-xs font-semibold text-slate-400 w-[200px]">KARYAWAN</th>
                  {WEEK_DAYS.map(day => (
                    <th key={day.key} className="p-4 text-center text-xs font-semibold text-slate-400">
                      {day.name}
                    </th>
                  ))}
                </tr>
