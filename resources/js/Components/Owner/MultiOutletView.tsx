  ...
  return (
    <Screen title="Perbandingan Outlet">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {outlets.map(outlet => (
          <Glass key={outlet.id} className="p-6 relative overflow-hidden group hover:border-white/20 transition-colors">
            <div className="flex justify-between items-start mb-4">
              ...
              <div className={`flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full ${
                outlet.growth >= 0 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'
              }`}>
                {outlet.growth >= 0 ? <TrendingUp className="size-3" /> : <TrendingDown className="size-3" />}
              ...
              <div key={outlet.id}>
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <div className="flex gap-4">
                    <span className="text-amber-400 font-semibold w-24 text-right">{formatRp(outlet.revenue)}</span>
