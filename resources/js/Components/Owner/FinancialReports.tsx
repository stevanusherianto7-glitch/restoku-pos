...
export function FinancialReports() {
  ...
  return (
    <Screen title="Laporan Keuangan" action={
      <div className="flex gap-2">
        <Button variant="outline" className="gap-2 text-xs"><FileText className="size-3" /> PDF</Button>
        <Button variant="outline" className="gap-2 text-xs"><FileSpreadsheet className="size-3" /> Excel</Button>
        <Button variant="outline" className="gap-2 text-xs"><FileJson className="size-3" /> CSV</Button>
      </div>
    }>
            ...
            {revenueBreakdown.map(item => (
              <div key={item.label}>
                ...
                <div className="w-full h-1.5 rounded-full bg-white/5">
                  <div className="h-full rounded-full bg-emerald-500/60" style={{ width: `${item.pct}%` }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Glass className="p-5 border-amber-500/30 bg-amber-500/5">
              <h4 className="text-xs font-semibold text-amber-500 uppercase tracking-wider mb-2">Laba Bersih</h4>
              <div className="text-2xl font-bold text-amber-400 mb-1">{formatRp(netProfit)}</div>
              <div className="text-sm text-slate-400">Margin: <span className="text-amber-300 font-medium">{netProfitPct}%</span></div>
            </Glass>
            
            <Glass className="p-5">
              <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Pajak Terkumpul</h4>
