            ...
            <tbody className="divide-y divide-white/5">
              {topMenu.map((item, idx) => (
                <tr key={item.id} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right text-emerald-400 font-medium">{formatRp(item.revenue)}</td>
                  <td className="px-4 py-3 text-right">{formatRp(item.hpp)}</td>
                  <td className="px-4 py-3 text-center">
                    <div className="flex items-center justify-center gap-2">
                      <Badge tone={item.foodCost > 40 ? "red" : item.foodCost > 35 ? "amber" : "emerald"}>
                        {item.foodCost.toFixed(1)}%
                      </Badge>
                      {item.foodCost > 40 && (
                        <span title="Food cost terlalu tinggi">
                          <AlertTriangle className="size-4 text-red-400" aria-label="Food cost terlalu tinggi" />
                        </span>
                      )}
                    </div>
                  </td>
