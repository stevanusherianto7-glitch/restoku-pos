      ...
      <div className="max-w-3xl space-y-6">
        ...
        <Glass className="p-6 border-amber-500/20">
            ...
            {alerts.map(alert => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg ${alert.tone === 'red' ? 'bg-red-500/20' : 'bg-amber-500/20'}`}>
                    <Package className={`size-4 ${alert.tone === 'red' ? 'text-red-400' : 'text-amber-400'}`} />
                  </div>
                  <div>
                    <div className={`font-semibold ${alert.tone === 'red' ? 'text-red-200' : 'text-amber-200'}`}>
                      {alert.item}
                    </div>
                    <div className={`text-sm mt-0.5 ${alert.tone === 'red' ? 'text-red-300/80' : 'text-amber-300/80'}`}>
          ...
          <div className="mt-6 p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/20 flex gap-3 items-center">
            ...
            <p className="text-sm font-medium text-emerald-200">Semua bahan baku lainnya dalam kondisi aman.</p>
          </div>
        </Glass>
        ...
        <Glass className="p-6 bg-white/5 border border-white/5 flex items-center justify-center text-center">
          <div className="max-w-xs">
            ...
            <p className="text-xs text-slate-500">Anda masuk sebagai Owner. Restock dan pengelolaan inventaris dilakukan oleh Admin/Dapur.</p>
          </div>
        </Glass>
      </div>
    </Screen>
