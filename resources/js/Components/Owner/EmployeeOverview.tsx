    ...
    <Screen title="Ringkasan Karyawan">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-4xl">
        ...
        <Glass className="p-6">
          ...
          <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Perbaikan per Departemen</h4>
          <div className="space-y-4">
            {departments.map(dept => (
              <div key={dept.name}>
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-slate-300">{dept.name} <span className="text-slate-500 text-xs ml-1">({dept.count} org)</span></span>
                  <span className="text-white font-medium">{formatRp(dept.salary)}</span>
                </div>
                <div className="w-full h-1.5 rounded-full bg-white/5">
                  <div className={`h-full rounded-full ${dept.color}`} style={{ width: `${(dept.salary / totalSalary) * 100}%` }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-3">
              <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20 flex gap-3 items-start">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-300/70 mt-0.5">Ada 2 karyawan yang masa kontraknya akan berakhir pada Agustus 2026. Admin telah diberitahu.</p>
                </div>
              </div>
            </div>
          </Glass>
          ...
          <Glass className="p-6 bg-white/5 border border-white/5 flex items-center justify-center text-center">
            <div className="max-w-xs">
              ...
              <p className="text-xs text-slate-500">Anda masuk sebagai Owner. Pengelolaan data karyawan, shift, dan payroll hanya dapat dilakukan oleh Admin/HRD.</p>
            </div>
          </Glass>
        </div>
