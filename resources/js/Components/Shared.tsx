...
export function useTenantSettings() {
        ...
        if (savedEmployees) {
            ...
            if (savedKasir || savedKitchen || savedWaiter || savedManager) {
                const migrated: StaffMember[] = [
                    ...
                    {
                        id: '2',
                        name: savedKitchen || 'DEDI CAHYONO',
                        role: 'kitchen',
                        pin: savedPinKitchen || '111111',
                    },
                    { id: '3', name: savedWaiter || 'SARI PERTIWI', role: 'waiter', pin: savedPinWaiter || '654321' },
                    {
                        id: '4',
                        name: savedManager || 'AGUS SETIAWAN',
                        role: 'manager',
                        pin: savedPinManager || '999999',
                    },
                ];
     ...
     */
    const saveSettings = (name: string, logo: string, image: string | null, ownerName: string) => {
...
export function RestokuLogo({ className = 'size-5' }: { className?: string }) {
    return (
        <svg viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
            ...
            <path
                d="M32 56C32 44 40 36 50 36C60 36 68 44 68 56"
                stroke="url(#logo-grad)"
                strokeWidth="6"
                strokeLinecap="round"
            />
            ...
            {/* Cloche Base block */}
            <path
                d="M32 66V70C32 72.76 34.24 75 37 75H63C65.76 75 68 72.76 68 70V66"
                stroke="url(#logo-grad)"
                strokeWidth="6"
                strokeLinecap="round"
                fill="url(#logo-grad)"
...
export function RestokuWordmark({ className = 'h-7 w-auto' }: { className?: string }) {
    return <img src="/images/restoku_logo.png" alt="Restoku" className={`select-none ${className}`} />;
}
