...
export const MOCK_GALLERY_EVENTS: GalleryEvent[] = [
    {
        ...
        photos: [
            {
                id: 'p1',
                path: 'https://images.unsplash.com/photo-1516450360452-9312f5e86fc7?q=80&w=800&auto=format&fit=crop',
            },
            {
                id: 'p2',
                path: 'https://images.unsplash.com/photo-1501386761578-eac5c94b800a?q=80&w=800&auto=format&fit=crop',
            },
        ],
    },
    {
        ...
        photos: [
            {
                id: 'p3',
                path: 'https://images.unsplash.com/photo-1555939594-58d7cb561ad1?q=80&w=800&auto=format&fit=crop',
            },
        ],
    },
    {
        ...
        photos: [
            {
                id: 'p4',
                path: 'https://images.unsplash.com/photo-1514933651103-005eec06c04b?q=80&w=800&auto=format&fit=crop',
            },
            {
                id: 'p5',
                path: 'https://images.unsplash.com/photo-1552566626-52f8b828add9?q=80&w=800&auto=format&fit=crop',
            },
        ],
    },
];
...
function Glass({ children, className = '' }: { children: React.ReactNode; className?: string }) {
    return (
        <div className={`rounded-2xl border border-white/5 bg-[#09090b]/80 shadow-sm backdrop-blur-xl ${className}`}>
            {children}
        </div>
    );
}
            ...
            <div className="flex-1 overflow-y-auto p-6 max-w-6xl mx-auto w-full">
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {events.map((event) => (
                        <div
                            key={event.id}
                            className="group cursor-pointer rounded-2xl overflow-hidden border border-white/10 bg-white/5 hover:border-white/20 transition-all duration-300"
                            ...
                            {currentEvent.photos.length > 1 && (
                                <>
                                    ...
                                    <button
                                        ...
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            setPhotoIndex((i) => Math.min(currentEvent.photos.length - 1, i + 1));
                                        }}
                                ...
                                <div className="grid grid-cols-4 gap-2 mt-auto">
                                    {currentEvent.photos.map((p, idx) => (
                                        ...
                                        >
                                            <img
                                                src={p.path}
                                                alt={`${currentEvent.title} — foto ${idx + 1}`}
            ...
            actions={
                <div className="flex gap-2">
                    <button
                        ...
                        className="flex items-center gap-2 rounded-xl bg-white/10 px-4 py-2 text-sm font-semibold text-white hover:bg-white/20 border border-white/10 transition-colors"
                    >
            ...
            <div className="grid gap-4">
                {MOCK_GALLERY_EVENTS.map((event) => (
                    <Glass key={event.id} className="p-4 flex items-center justify-between group">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-slate-400 mt-1">
                                    {event.event_date} · {event.photos.length} foto
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                            <span
                                className={`px-2.5 py-1 rounded-md text-[10px] font-semibold uppercase ${event.status === 'published' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'}`}
                            >
                                {event.status}
                            </span>
                            ...
                            <button className="p-2 text-slate-400 hover:text-red-400 bg-white/5 rounded-lg border border-white/5 hover:bg-red-500/10">
                                Hapus
                            </button>
                        </div>
                    </Glass>
            ...
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                <Glass className="p-6">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="text-xs bg-white/10 px-2 py-1 rounded font-mono text-slate-400">
                    ...
                    <div className="space-y-3">
                        <label className="text-sm font-medium text-slate-300">
                            Foto Terunggah (Drag untuk mengurutkan)
                        </label>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            {photos.map((p, i) => (
                                <div
                                    key={p.id}
                                    className="group relative aspect-square rounded-xl overflow-hidden border border-white/10 bg-slate-800"
                                >
                                    <img
                                        src={p.path}
                                        alt={`Foto terunggah ${i + 1}`}
                                        className="w-full h-full object-cover"
                                    />
