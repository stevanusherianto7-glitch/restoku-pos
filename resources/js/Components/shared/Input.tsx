...
export function Input({ className = "", error, label, hint, id, ...props }: InputProps) {
  ...
  return (
    <div className="w-full space-y-1.5">
      {label && (
        <label htmlFor={inputId} className={`block text-xs font-semibold ml-0.5 ${isLight ? "text-slate-700" : "text-slate-400"}`}>
          {label}
      ...
      {hint && !error && <p className={`text-xs ml-0.5 ${isLight ? "text-slate-500" : "text-slate-500"}`}>{hint}</p>}
    </div>
  );
}
