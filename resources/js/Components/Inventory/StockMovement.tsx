...
export function StockMovement({ initialTab = 'riwayat' }: { initialTab?: 'riwayat' | 'masuk' | 'waste' | 'opname' }) {
    ...
    const transactions = [
        ...
        },
        {
            ...
            unit: 'kg',
            pic: 'System',
            note: 'Auto-deduct Order #1002',
            cost: 12000,
        },
        {
            id: 'TRX-003',
            date: '11 Jul 2026 20:00',
            type: 'waste',
            ...
            unit: 'butir',
            pic: 'Budi (Kitchen)',
            note: 'Pecah saat dipindah',
            cost: 2000,
        },
        {
            id: 'TRX-004',
            date: '10 Jul 2026 23:30',
            type: 'adjustment',
    ...
    const opnames = [
        ...
        {
            id: 'OPN-002',
            title: 'Opname Bulanan (Bahan Segar)',
            date: '30 Jun 2026',
            status: 'completed',
            pic: 'Andi (Admin)',
            discrepancy: 0,
    ...
    const getTypeBadge = (type: string) => {
        switch (type) {
            case 'in':
                return <Badge tone="emerald">Masuk (In)</Badge>;
            case 'out':
                return <Badge tone="blue">Keluar (Out)</Badge>;
            case 'waste':
                return <Badge tone="red">Rusak (Waste)</Badge>;
            case 'adjustment':
                return <Badge tone="amber">Adjustment</Badge>;
            default:
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'completed':
                return <Badge tone="emerald">Selesai</Badge>;
            case 'in_progress':
                return <Badge tone="blue">Berjalan</Badge>;
            case 'draft':
                                    ...
                                    <td colSpan={7} className="px-4 py-8 text-center text-slate-500">
                                        <div className="flex flex-col items-center gap-2">
                                            ...
                                            <p>
                                                ...
                                                {activeTab === 'masuk' ? 'Barang Masuk' : 'Barang Rusak'}" untuk
                                                menambah data.
                                            </p>
                                        </div>
                                    </td>
                            ...
                            {activeTab === 'riwayat' &&
                                transactions.map((item) => (
                                    <tr key={item.id} className="hover:bg-white/5 transition-colors">
                                        ...
                                        <td
                                            className={`px-4 py-3 font-semibold ${item.type === 'in' ? 'text-emerald-400' : item.type === 'out' || item.type === 'waste' ? 'text-red-400' : 'text-amber-400'}`}
                                        >
                                            {item.type === 'in' ? '+' : '-'}
                                            {Math.abs(item.qty)} {item.unit}
                                        </td>
