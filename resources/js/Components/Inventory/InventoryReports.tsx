        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-300">
            ...
            <tbody className="divide-y divide-white/5">
              {activeTab === "hpp" && hppReports.map((item) => (
                <tr key={item.id} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-4 py-3 text-center">
                    <Badge tone={item.percentage > 35 ? "red" : item.percentage > 25 ? "emerald" : "blue"}>
                      {item.percentage}%
                    </Badge>
                  </td>
                  <td className="px-4 py-3 text-right font-semibold text-emerald-400">{formatRp(item.profit)}</td>
                </tr>
              ))}

              {activeTab === "mutasi" && mutationReports.map((item) => (
                <tr key={item.id} className="hover:bg-white/5 transition-colors">
                  <td className="px-4 py-3 text-slate-400">{item.date}</td>
                  <td className="px-4 py-3 font-medium text-white">{item.item}</td>
                  <td className="px-4 py-3 text-center text-emerald-400 font-medium">+{item.in} <span className="text-xs font-normal text-slate-500">{item.unit}</span></td>
                  <td className="px-4 py-3 text-center text-red-400 font-medium">-{item.out} <span className="text-xs font-normal text-slate-500">{item.unit}</span></td>
                  <td className="px-4 py-3 text-center text-white font-semibold">{item.balance} <span className="text-xs font-normal text-slate-500">{item.unit}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
