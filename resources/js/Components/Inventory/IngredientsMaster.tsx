...
export function IngredientsMaster({ initialTab = "bahan" }: { initialTab?: "bahan" | "kategori" | "supplier" }) {
  ...
  const ingredients = [
    { id: 1, sku: "ING-001", name: "Beras Premium", category: "Bahan Kering", stock: 25, min_stock: 10, unit: "kg", recipe_unit: "gr", conversion: 1000, cost: 12000, method: "FIFO" },
    { id: 2, sku: "ING-002", name: "Daging Ayam", category: "Bahan Segar", stock: 3, min_stock: 5, unit: "kg", recipe_unit: "gr", conversion: 1000, cost: 35000, method: "FEFO" },
    { id: 3, sku: "ING-003", name: "Minyak Goreng", category: "Bahan Kering", stock: 10, min_stock: 5, unit: "liter", recipe_unit: "ml", conversion: 1000, cost: 18000, method: "FIFO" },
  ];
  ...
  return (
    <Screen title="Master Bahan Baku & Supplier" action={
      <Button className="gap-2">
        <Plus className="size-4" /> 
        {activeTab === "bahan" ? "Tambah Bahan Baku" : activeTab === "kategori" ? "Tambah Kategori" : "Tambah Supplier"}
      </Button>
    }>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-300">
            ...
            <tbody className="divide-y divide-white/5">
              {activeTab === "bahan" && ingredients.map((item) => (
                <tr key={item.id} className="hover:bg-white/5 transition-colors">
                  <td className="px-4 py-3">
                    <div className="font-medium text-white">{item.name}</div>
                    <div className="text-[10px] text-slate-500 font-mono mt-0.5">{item.sku}</div>
                  </td>
                  <td className="px-4 py-3"><Badge tone="emerald">{item.category}</Badge></td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <span>{item.stock}</span>
                      {item.stock <= item.min_stock && <Badge tone="amber">Low</Badge>}
                    </div>
                    <div className="text-[10px] text-slate-500 mt-0.5">Min: {item.min_stock} {item.unit}</div>
                  </td>
                  <td className="px-4 py-3">
                    <div>{item.unit}</div>
                    <div className="text-[10px] text-slate-500 mt-0.5">1 {item.unit} = {item.conversion} {item.recipe_unit}</div>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-2">
                      <button className="p-1 hover:bg-white/10 rounded-md transition-colors text-slate-400 hover:text-white"><Edit2 className="size-4" /></button>
                      <button className="p-1 hover:bg-red-500/20 rounded-md transition-colors text-slate-400 hover:text-red-400"><Trash2 className="size-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
              ...
              {activeTab === "supplier" && suppliers.map((item) => (
                <tr key={item.id} className="hover:bg-white/5 transition-colors">
                  <td className="px-4 py-3 font-medium text-white">{item.name}</td>
                  <td className="px-4 py-3 text-slate-400">{item.contact}</td>
                  <td className="px-4 py-3"><Badge tone="blue">{item.terms}</Badge></td>
