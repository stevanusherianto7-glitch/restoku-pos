...
import { 
  Users, UserPlus, Search, ChevronRight, FileText, CheckCircle2, 
  AlertTriangle, Clock, Briefcase, FileBadge, CalendarOff, ShieldAlert,
  Save, Upload, MapPin
} from "lucide-react";
...
const MOCK_EMPLOYEES: Employee[] = [
  { id: 1, nik: "3174123456789012", name: "Budi Hartono", role: "Waiter", dept: "Service", status: "Kontrak", phone: "0812-3456-7890", email: "budi@restoku.app", joinDate: "01/01/2026", contractEnd: "30 Agt 2026" },
  { id: 2, nik: "3174123456789013", name: "Sari Pertiwi", role: "Chef", dept: "Kitchen", status: "Permanent", phone: "0812-1111-2222", email: "sari@restoku.app", joinDate: "15/03/2024", contractEnd: "-" },
  { id: 3, nik: "3174123456789014", name: "Andi Saputra", role: "Kitchen Helper", dept: "Kitchen", status: "Probation", phone: "0812-3333-4444", email: "andi@restoku.app", joinDate: "01/06/2026", contractEnd: "15 Agt 2026" }
];
...
export function EmployeeMaster() {
  ...
  return (
    <Screen 
      ...
      actions={<Button onClick={() => { setSelectedUser(null); setView("form"); }}><UserPlus className="size-4 mr-2" /> Tambah Karyawan</Button>}
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <Glass className="p-4 flex items-center justify-between">
          ...
          <div className="size-10 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-400"><Users className="size-5" /></div>
        </Glass>
        <Glass className="p-4 flex items-center justify-between">
          <div>
          ...
          <div className="size-10 rounded-full bg-emerald-500/20 flex items-center justify-center text-emerald-400"><CheckCircle2 className="size-5" /></div>
        </Glass>
        <Glass className="p-4 flex items-center justify-between">
          <div>
          ...
          <div className="size-10 rounded-full bg-amber-500/20 flex items-center justify-center text-amber-400"><CalendarOff className="size-5" /></div>
        </Glass>
        <Glass className="p-4 flex items-center justify-between border-red-500/30">
          <div>
            ...
            <p className="text-2xl font-bold text-red-400 mt-1">2 <span className="text-sm font-normal">bln dpn</span></p>
          </div>
          <div className="size-10 rounded-full bg-red-500/20 flex items-center justify-center text-red-400"><AlertTriangle className="size-5" /></div>
        </Glass>
      </div>
        ...
        <Glass className="p-5 lg:col-span-2">
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="w-full bg-white/5 rounded-full h-2"><div className="bg-blue-500 h-2 rounded-full" style={{ width: '40%' }}></div></div>
            </div>
            <div>
              <div className="flex justify-between text-sm mb-1">
        ...
        <Glass className="p-5">
          ...
          <div className="space-y-3">
            <div className="bg-white/5 p-3 rounded-xl border border-white/5">
              ...
              <Button variant="outline" className="w-full text-xs py-1">Evaluasi</Button>
            </div>
            <div className="bg-white/5 p-3 rounded-xl border border-white/5">
              <div className="flex items-center gap-2 mb-1">
              ...
              <Button variant="outline" className="w-full text-xs py-1">Perpanjang / Tdk</Button>
            </div>
          </div>
        </Glass>
      </div>
      ...
      <Glass className="p-0 overflow-hidden">
        <div className="p-4 border-b border-white/5 flex items-center justify-between">
          <div className="relative w-64">
            ...
            <input type="text" placeholder="Cari karyawan..." className="w-full bg-white/5 border border-white/10 rounded-lg pl-9 pr-4 py-2 text-sm text-white focus:outline-none focus:border-blue-500 transition-colors" />
          </div>
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-white/5">
            {MOCK_EMPLOYEES.map(emp => (
              <tr key={emp.id} className="hover:bg-white/[0.02] transition-colors group cursor-pointer" onClick={() => { setSelectedUser(emp); setView("form"); }}>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <div className="size-8 rounded-full bg-slate-700 flex items-center justify-center font-medium text-white">{emp.name.charAt(0)}</div>
                    <div>
                ...
                <td className="px-4 py-3">
                  <Badge tone={emp.status === "Permanent" ? "emerald" : emp.status === "Kontrak" ? "blue" : "amber"}>{emp.status}</Badge>
                </td>
                <td className="px-4 py-3 text-slate-400">{emp.contractEnd}</td>
                <td className="px-4 py-3 text-right">
                  <Button variant="ghost" className="opacity-0 group-hover:opacity-100"><ChevronRight className="size-4" /></Button>
                </td>
              </tr>
            ))}
          </tbody>
    ...
    <Screen 
      ...
      actions={
        <>
          ...
          <Button><Save className="size-4 mr-2" /> Simpan Data</Button>
        </>
      }
    >
      <div className="flex flex-col md:flex-row gap-6 h-full">
        ...
        <div className="flex-1 space-y-6">
          {activeTab === "Profil" && (
            <Glass className="p-6">
              ...
              <div className="flex gap-6 mb-8">
                ...
                <div className="flex-1 grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input type="email" defaultValue={user?.email} placeholder="cth. budi@restoku.app" />
                  </div>
                  <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-slate-400 mb-1">Tempat Lahir</label>
                  <Input defaultValue={user ? "Jakarta" : ""} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-400 mb-1">Tanggal Lahir</label>
          ...
          {activeTab === "Identitas & Alamat" && (
            <>
              <Glass className="p-6 mb-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-slate-400 mb-1">NIK (KTP)</label>
                    <Input defaultValue={user?.nik} />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-slate-400 mb-1">No. Kartu Keluarga</label>
                    <Input defaultValue={user?.nik} />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-medium text-slate-400 mb-1">NPWP</label>
                      <Input defaultValue={user ? "98.765.432.1-000.000" : ""} />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-slate-400 mb-1">Status PTKP (PPh 21)</label>
                      <select className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500" defaultValue="TK/0">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-medium text-slate-400 mb-1">BPJS Ketenagakerjaan</label>
                      <Input defaultValue={user ? "1234567890" : ""} />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-slate-400 mb-1">BPJS Kesehatan</label>
              ...
              <Glass className="p-6">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-medium text-slate-400 mb-1">Alamat Sesuai KTP</label>
                    <textarea className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500" rows={2} defaultValue="Jl. Merdeka No. 123, Jakarta Pusat"></textarea>
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-slate-400 mb-1">Alamat Domisili</label>
                    <textarea className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500" rows={2} defaultValue="Sama dengan KTP"></textarea>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-medium text-slate-400 mb-1">Nama Kontak Darurat</label>
                      <Input defaultValue="Siti (Ibu)" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-slate-400 mb-1">No. HP Kontak Darurat</label>
            ...
            <Glass className="p-6">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div>
                  ...
                  <select className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500" defaultValue={user?.role}>
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-400 mb-1">Status Karyawan</label>
                  <select className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500" defaultValue={user?.status}>
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-400 mb-1">Tanggal Masuk</label>
                  <Input type="date" defaultValue="2026-01-01" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-400 mb-1">Tanggal Keluar (Resign)</label>
                  <Input type="date" />
                </div>
              </div>
              <h3 className="text-sm font-semibold text-white mb-4">Informasi Penggajian & Rekening Bank</h3>
              <div className="grid grid-cols-4 gap-4">
                <div>
                  <label className="block text-xs font-medium text-slate-400 mb-1">Gaji Pokok (Basic Salary)</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-400 mb-1">No. Rekening</label>
          ...
          {activeTab === "Dokumen" && (
            <Glass className="p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <Button variant="outline" size="sm"><Upload className="size-4 mr-2" /> Upload Dokumen</Button>
              </div>
              <div className="space-y-3">
          ...
          {activeTab === "Kontrak" && (
            <Glass className="p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <Button variant="outline" size="sm"><Briefcase className="size-4 mr-2" /> Buat Kontrak Baru</Button>
              </div>
              <div className="space-y-4">
                <div className="p-4 rounded-xl border border-white/10 bg-white/5">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <h4 className="font-semibold text-white flex items-center gap-2">Contract 1 <Badge tone="blue">Aktif</Badge></h4>
          ...
          {activeTab === "Kedisiplinan (SP)" && (
            <Glass className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-lg font-semibold text-white">Catatan Pelanggaran & Surat Peringatan</h2>
                <Button variant="outline" size="sm"><ShieldAlert className="size-4 mr-2" /> Catat Pelanggaran</Button>
              </div>
