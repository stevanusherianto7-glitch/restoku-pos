          ...
          <Glass className="p-6 h-full">
            ...
            <div className="space-y-4">
              {[
                ...
                { date: "Jumat, 10 Jul 2026", shift: "Pagi (07:00 - 15:00)", status: "pending_publish" },
              ].map((item, idx) => (
                <div key={idx} className={`p-4 rounded-xl border ${item.date.includes("6 Jul") ? 'bg-emerald-900/20 border-emerald-500/50' : 'bg-slate-900/50 border-white/10'} flex items-center justify-between`}>
                  <div>
                    <div className="font-medium text-white mb-1">{item.date}</div>
                    <div className={`text-sm ${item.status === 'off' ? 'text-slate-500' : 'text-slate-300'}`}>
                      {item.shift}
                    </div>
                  </div>
