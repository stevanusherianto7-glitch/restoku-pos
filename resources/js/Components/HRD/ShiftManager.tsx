                    ...
                    <table className="w-full text-sm text-left border-collapse min-w-[800px]">
                        <thead>
                            <tr className="border-b border-white/10 text-slate-400">
                                ...
                                {days.map((d) => (
                                    <th key={d} className="p-3 font-medium text-center min-w-[120px]">
                                        {d}
                                    </th>
                                ))}
                            </tr>
                                        ...
                                        <td colSpan={8} className="p-3">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs text-slate-400">
                                                    ({data.time} | Toleransi Telat: {data.tolerance})
                                                </span>
                                            </div>
                                        </td>
                                    </tr>
                                            ...
                                            {assigned.map((staff, idx) => (
                                                <td key={idx} className="p-2 border-l border-white/5 text-center">
                                                    {staff ? (
                                                        ...
                                                        <div
                                                            className={`border border-dashed rounded-lg p-3 flex flex-col items-center justify-center cursor-pointer transition-colors ${getAlert(shiftName as keyof typeof scheduleData, roleName, idx) ? 'border-red-500/50 hover:bg-red-500/10' : 'border-white/20 hover:bg-white/5'}`}
                                                        >
                                                            {getAlert(
                                                                shiftName as keyof typeof scheduleData,
                                                                roleName,
                                                                idx,
                                                            ) ? (
