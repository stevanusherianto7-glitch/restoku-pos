...
import { 
  Calculator, CheckCircle2, ChevronRight, Download, FileText, 
  Search, Send, DollarSign, Calendar
} from "lucide-react";

const MOCK_PAYROLL = [
  { id: 1, user: { name: "Budi Hartono", role: "Waiter", dept: "Service" }, period: "Juli 2026", base: 3500000, allowances: 800000, overtime: 200000, deductions: 338333, net: 4161667, status: "draft" },
  { id: 2, user: { name: "Sari Pertiwi", role: "Chef", dept: "Kitchen" }, period: "Juli 2026", base: 6000000, allowances: 1000000, overtime: 500000, deductions: 500000, net: 7000000, status: "approved" },
  { id: 3, user: { name: "Andi Saputra", role: "Kitchen Helper", dept: "Kitchen" }, period: "Juli 2026", base: 2800000, allowances: 500000, overtime: 0, deductions: 100000, net: 3200000, status: "paid" },
];
...
export function PayrollManagement() {
  ...
  return (
    <Screen 
      ...
      actions={
        <>
          <Button variant="outline"><Calendar className="size-4 mr-2" /> Juli 2026</Button>
          <Button><Calculator className="size-4 mr-2" /> Generate Gaji Bulan Ini</Button>
        </>
      }
    >
      ...
      <Glass className="p-0 overflow-hidden relative">
        <div className="p-4 border-b border-white/5 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm"><Send className="size-4 mr-2" />Kirim Semua Slip via WA</Button>
          </div>
        </div>
            ...
            {payrolls.map(pr => (
              <tr key={pr.id} className="hover:bg-white/[0.02] transition-colors">
                <td className="px-4 py-3">
                  ...
                  <div className="text-xs text-slate-500">{pr.user.role} · {pr.user.dept}</div>
                </td>
                <td className="px-4 py-3 text-right text-slate-300">{formatCurrency(pr.base)}</td>
                <td className="px-4 py-3 text-right text-emerald-400">+{formatCurrency(pr.allowances + pr.overtime)}</td>
                ...
                <td className="px-4 py-3 text-center">
                  <Badge tone={pr.status === "paid" ? "emerald" : pr.status === "approved" ? "blue" : "slate"}>
                    {pr.status === "paid" ? "Terbayar" : pr.status === "approved" ? "Disetujui" : "Draft"}
                  </Badge>
                </td>
                <td className="px-4 py-3 text-right space-x-2">
                  ...
                  {pr.status === "draft" && (
                    <Button size="sm" onClick={() => approvePayroll(pr.id)}>Approve</Button>
                  )}
                  {pr.status === "approved" && (
                    <Button size="sm" className="bg-emerald-500 hover:bg-emerald-600" onClick={() => markPaid(pr.id)}>Bayar</Button>
                  )}
                </td>
              </tr>
            ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-white text-black w-full max-w-sm rounded-lg shadow-2xl overflow-hidden flex flex-col">
            <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              ...
              <div className="flex gap-2">
                <Button variant="ghost" size="sm" className="text-slate-600 hover:bg-slate-200 h-8 w-8 p-0 rounded-full"><Download className="size-4" /></Button>
                <Button variant="ghost" size="sm" onClick={() => setSelectedPayslip(null)} className="text-slate-600 hover:bg-slate-200 px-3 h-8 rounded-full">Tutup</Button>
              </div>
            </div>
            ...
            <div className="p-6 font-mono text-sm leading-relaxed overflow-y-auto max-h-[70vh]">
              ...
              <div className="border-y border-dashed border-slate-300 py-3 mb-4 space-y-1">
                <div className="flex justify-between"><span>Nama:</span> <span className="font-bold">{selectedPayslip.user.name}</span></div>
                <div className="flex justify-between"><span>Jabatan:</span> <span>{selectedPayslip.user.role}</span></div>
                <div className="flex justify-between"><span>Dept:</span> <span>{selectedPayslip.user.dept}</span></div>
              </div>
              
              <div className="mb-4">
                <div className="font-bold mb-2">PENERIMAAN</div>
                <div className="flex justify-between"><span>Gaji Pokok</span> <span>{formatCurrency(selectedPayslip.base)}</span></div>
                <div className="flex justify-between"><span>Tunjangan</span> <span>{formatCurrency(selectedPayslip.allowances)}</span></div>
                <div className="flex justify-between"><span>Lembur</span> <span>{formatCurrency(selectedPayslip.overtime)}</span></div>
                <div className="border-t border-slate-300 mt-2 pt-2 flex justify-between font-bold">
                  <span>Total Penerimaan</span> <span>{formatCurrency(selectedPayslip.base + selectedPayslip.allowances + selectedPayslip.overtime)}</span>
                </div>
              </div>

              <div className="mb-6">
                <div className="font-bold mb-2">POTONGAN</div>
                <div className="flex justify-between text-red-600"><span>BPJS/Absen</span> <span>-{formatCurrency(selectedPayslip.deductions)}</span></div>
                <div className="flex justify-between text-red-600"><span>PPh 21</span> <span>-Rp 0</span></div>
                <div className="border-t border-slate-300 mt-2 pt-2 flex justify-between font-bold text-red-600">
                  <span>Total Potongan</span> <span>-{formatCurrency(selectedPayslip.deductions)}</span>
                </div>
              </div>
